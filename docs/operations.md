...
- **Expected depth: one message per terminal job, monotonically increasing.** Every job that reaches `completed` or `failed` adds one, and nothing removes any. Depth therefore tracks total completed-plus-failed jobs since the deploy, and a rising number is the system working, not a fault. What would be a fault is a depth that stops rising while jobs keep finishing.
...
- **Clearing it before the consumer exists is safe, and is the intended response to the bound.** The authoritative record of every outcome is the `video_jobs` row plus its outbox row; the message is only the announcement. `rabbitmqctl purge_queue video.jobs.terminal.events.v1` discards buffered announcements for jobs that finished before anyone was listening — do not purge once a consumer is deployed, and expect that consumer's first run to see a burst of buffered outcomes if the queue was not purged before it started.
