...
The three processes have deliberately different configuration surfaces:

| | `cmd/api` | `cmd/worker` | `cmd/notifier` |
|---|---|---|---|
| `IDENTITY_POSTGRES_DSN`, `IDENTITY_JWT_SIGNING_KEY` | **required** | **not read** — the worker makes no access-control decision | **not read** — same reason |
| `VIDEO_POSTGRES_DSN` | required | required | **not read** — it never touches a `VideoJob` |
| `NOTIFICATION_POSTGRES_DSN` | **required** | **not read** — the worker registers no preference and reads none | **required** |
| `REDIS_ADDR` | required | required (status cache, worker leases, and clearing failed-job idempotency keys) | **not read** — it holds no lease and caches nothing |
| `VIDEO_MINIO_ENDPOINT` / `_ACCESS_KEY` / `_SECRET_KEY` / `_BUCKET` | required | required | **not read** — it stores and fetches no artifact |
| `VIDEO_MINIO_PUBLIC_ENDPOINT`, `VIDEO_MINIO_PUBLIC_USE_SSL` | optional | optional, and **read even though unused** — see below | not read |
| `RABBITMQ_URL` | required | required | required |
| `NOTIFICATION_ALLOW_INSECURE_DESTINATIONS` | optional — **one variable, two readers** | not read | optional — the same variable, the same parser, the same default |
| `NOTIFICATION_WEBHOOK_MAX_ATTEMPTS`, `NOTIFICATION_WEBHOOK_TIMEOUT_SECONDS`, `NOTIFICATION_DELIVERY_RECLAIM_SECONDS` | not read | not read | optional, and **validated against one another at startup** — see below |
| `RATE_LIMIT_*` | optional | not read | not read |
| `PORT` / `GIN_MODE` | as below | not read — the worker serves no HTTP and exposes no port | not read — same |

`NOTIFICATION_ALLOW_INSECURE_DESTINATIONS` is the row that surprises people: it reads like a delivery concern, but `cmd/api` needs it too, because the destination policy is applied both when a preference is written and when its address is dialled. One variable, one parser, and a deployment whose two processes disagree either stores destinations it can never deliver to or refuses at dial what it accepted at write time. `cmd/worker` is the only process that does not read it.
...
| `NOTIFICATION_ALLOW_INSECURE_DESTINATIONS` | `false` | Relaxes the webhook destination policy's **scheme rule and address rule together** — with it, a destination may use `http` and may resolve to a private, loopback, or link-local address. Optional as of Phase 7's `add-notification-webhook-delivery`, read by `cmd/api` and `cmd/notifier` through one parser. One switch rather than two because the two relaxations are wanted in exactly the same situation, and splitting them would invite enabling half of it where neither belongs. A value that is *set* and not parseable as a boolean is a configuration error, never a silent `false`: reading a typo as `false` leaves a local stack refusing every destination with nothing to point at, and reading it as `true` would let a typo open the policy in production. **Set it in local development and the compose stack; never in production** — see "The destination policy" below for what it costs. |
| `NOTIFICATION_WEBHOOK_MAX_ATTEMPTS` | `3` | How many times one delivery is attempted before its outcome is recorded as `failed`. Optional, `cmd/notifier` only. Not independent of the reclaim bound — see below. |
| `NOTIFICATION_WEBHOOK_TIMEOUT_SECONDS` | `5` | Per-attempt timeout, and also the delivery client's transport timeout. Optional, `cmd/notifier` only. Not independent of the reclaim bound. |
| `NOTIFICATION_DELIVERY_RECLAIM_SECONDS` | `120` | How long an unresolved claim must be before another consumer may take it over. Optional, `cmd/notifier` only. **Validated at startup against the other two**: a bound below twice one claimant's maximum hold fails startup naming both values. |
...
`cmd/notifier` reads the narrowest surface of the three, and applies the same rules to it. Configuration first, all of it, before any I/O: `RABBITMQ_URL`, then `NOTIFICATION_POSTGRES_DSN`, then the destination policy, then the delivery budget — so a value the process will refuse to start on is reported as a bad value rather than as a failure to reach something it was never configured to reach. Only then is the pool opened, migrated, and pinged. The broker is deliberately *not* a startup gate: the consumer dials with bounded backoff and redeclares the topology after every dial, so neither this process's startup nor the worker's depends on the other's order. It creates no directory, writes nothing to disk, and opens no port.

#### The destination policy

A webhook destination is judged **twice**: once by `cmd/api` when the preference is written, and again by `cmd/notifier` against the **resolved network address** when the connection is opened. Neither point is redundant. Write-time alone cannot survive a hostname that resolves elsewhere later, nor a policy tightened after the row was stored. Dial-time alone silently accepts a destination that will never be delivered to, which to its owner is indistinguishable from one that works.
...
### RabbitMQ — Topologies declared, published to by two outbox relays, dispatch consumed by `cmd/worker` and terminal events by `cmd/notifier` (Phase 6; terminal events Phase 7)
...
**Four connections across the three processes.** `cmd/api` opens one, for the dispatch outbox relay publishing `video_job.queued.v2` events (`add-videojob-source-key-and-outbox-relay`). `cmd/worker` opens two: the consumer that reads those dispatches (`migrate-upload-to-async-processing`), and — since `emit-videojob-terminal-events` — a second relay publishing `video_job.completed.v1` and `video_job.failed.v1`. `cmd/notifier` opens the fourth, the consumer that reads that terminal stream (`add-notification-webhook-delivery`). Each declares its topology after every successful dial, so no process depends on another having started first, and a broker recreated while one was disconnected gets its entities back. See "The outbox relays" and "The worker" below.
...
### Email delivery — Planned (Phase 7, `add-notification-email-delivery`)

An SMTP adapter behind the same `Deliverer` port the webhook channel already uses, plus a mail service in `docker-compose.yml` for local development. Until it ships, `email` is a **rejected** `Channel` value on `PUT /api/notification-preferences` — a channel a user can configure and that silently delivers nothing is worse than a rejected value. It also depends on `add-identity-user-registered-event`, since Notification never calls Identity: an address reaches it as a locally-projected `UserRegistered` event, not as a lookup.

**Webhook delivery is no longer planned — it shipped** (`add-notification-webhook-delivery`) and is documented above: `cmd/notifier` under "Docker" and "Environment Variables", the queue under "The terminal-event queue and its consumer", and the policy, budget, and operational signals under "The destination policy", "The delivery budget and the reclaim bound", and "Two signals this design deliberately leaves behind".
