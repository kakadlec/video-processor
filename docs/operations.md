...
| `NOTIFICATION_POSTGRES_DSN` | unset | PostgreSQL connection string for the Notification module's `NotificationPreference` repository (e.g. `postgres://user:pass@host:5432/identity?sslmode=disable` — the same instance and database as the other two by design; the split is one pool per bounded context, not one server per context). Required at `cmd/api` startup as of Phase 7's `add-notification-domain-and-preferences`, and connectivity-checked like the other two. Not read by `cmd/worker`. |
