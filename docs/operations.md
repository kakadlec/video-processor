...
**The three HTTP services all listen on 8080**, each inside its own container, and none of them publishes it. The gateway is the only *application* process that publishes a host port; the worker and the notifier listen only on their metrics-only ports, `:9102` and `:9103`, which nothing publishes either. The local compose stack also publishes the mail catcher's inspection port and, since `add-local-metrics-scraper`, a Prometheus server's — both development-only support services serving no application route and not part of any deployment.
...
- **`Migrate` takes a `pg_advisory_xact_lock`** (class `0x46494158`, object `1`) for the length of its transaction, unlike the identity adapter. `CREATE TABLE IF NOT EXISTS` does not serialize two *first-time* creates, so without it two `cmd/notification-api` replicas starting together against a fresh database can race to a catalog uniqueness violation — which at startup means a replica that refuses to boot. `internal/video/infrastructure/postgres`'s `Migrate` takes the same kind of lock, under object `2` in the same class, since `serialize-video-schema-migration`; the identity adapter is the one that still has no lock. If you add an advisory lock elsewhere, take a new object id under that same class; the comments in `internal/notification/infrastructure/postgres/migrate.go` and `internal/video/infrastructure/postgres/migrate.go` are the registry.
...
| PostgreSQL could not answer the claim step — the authoritative load before the claim, the claim itself, or the existence probe after a zero-row claim (`ErrJobClaimOutcomeUnknown`) | **Requeue**, then a 5s pause before the next delivery | not known to this run, which asserts nothing about it; the redelivery's own load and claim decide it — a `queued` row is claimed, anything else is dead-lettered. Only a claim that committed and lost its result necessarily leaves it `processing` with no lease, which the sweeper recovers | none; this run acquired no lease, read no source and cleared nothing |
| The claim step failed with an error the database *answered* — a row that will not reconstruct, a statement it refused (a half-applied migration, a missing privilege) | Reject → DLQ | unchanged, whatever its state; if that state is `queued`, nothing can advance it until the dispatch is replayed (see below) | untouched |
...
- an `error` naming whatever reached the adapter — with the database container stopped, a DNS `no such host` inside a `*pgconn.ConnectError`, raised by the authoritative load that precedes the claim, so no claim was attempted at all;
...
**A PostgreSQL outage before a claim is won no longer puts a dispatch here**: a claim step PostgreSQL could not answer is requeued (above). One **after** a won claim still does — a completion or failure write that could not commit is dead-lettered — but it leaves a `processing` row the sweeper recovers without the message, so that dead-letter needs no replay. Besides it, what lands here is the permanent cases: an undecodable body, a missing source key, a malformed or unknown job, a lost claim, a fenced terminal write, a claim step that failed with an error the database *answered*, a completion that would not commit; and from the notifier, an undecodable body or an unrecognized event type. **Read each message as a defect to look at, not as work to replay.** Every worker dead-letter also has an `error`-level `component: job_dispatch` record, which is usually the faster read: it carries the dispatch's `job_id` whenever the body decoded, and only the `error` for a body that did not. The message confirms what was actually sent.
...
`docker compose logs` also collects nginx, PostgreSQL, Redis, RabbitMQ, MinIO, the mail catcher, and Prometheus, none of which this repository builds and none of which emits JSON. Scope the command to the five application services before piping to `jq`, or it fails on the first line of somebody else's format.
...
**And a Grafana over it.** `docker-compose.yml` runs a `grafana` service reading that Prometheus, on its own loopback-bound port (`127.0.0.1:3000`), with the same standing as the mail catcher and the scraper: an image this repository does not produce, no gateway route, no deployment. Both the datasource and one dashboard — **FIAP X — Visão geral**, covering the job pipeline, the extraction and dispatch outcomes, the notifier's delivery time against its configured claim-hold budget, and the three HTTP services' rate and latency — are provisioned from files under `docker/grafana/`, mounted read-only. So the dashboard in the repository is the dashboard you get, UI edits are disabled, and a fresh volume changes nothing. Two of its panels read families that are **absent while the state they describe is empty** (`fiapx_video_jobs_oldest_in_state_age_seconds`, `fiapx_video_job_outbox_oldest_unpublished_age_seconds`): an empty panel there is the queue being clear, not the metric being broken. Alerting is not configured — there is no deployment to page.
...
The worker and the notifier answer the same way, on their own port and with no gateway refusal to speak of — nothing routes to either of them at all:
...
**A scrape is access-logged like any other request.** The probe routes are exempt from the access record because a prober arrives at a fixed interval forever; the local stack's Prometheus server scrapes at 15s, which is 5,760 records a day per service — the same order of magnitude as the already-exempted probe traffic. `add-local-metrics-scraper` revisited the exemption with that real number rather than leaving it for later and left it as is: extending the exemption to `/metrics` is a source change to three `logging.go` files and their tests, not a change to the local stack, so it stayed out of that change's scope. `/metrics` stays access-logged. The worker's and the notifier's own scrapes carry no such record at all, exempt or otherwise: neither process keeps an access log — there is no `logging.go` on the metrics-only listener to hold one — the same absence "Health and readiness probes" already names for every other request either process never answers.
...
| `fiapx_video_extraction_duration_seconds` | `worker` (declared in `internal/video/infrastructure/ffmpeg`, the only caller of `ExtractFrames`) | `outcome` = `success` / `failure` | How long one extraction actually takes, including a slow failure — ffmpeg hanging rather than exiting fast — which the dispatch counter below cannot distinguish from a fast one. |
| `fiapx_worker_dispatch_outcomes_total` | `worker` | `outcome` (one value per branch of `handle()`'s decision table — `claim_lost`, `claim_outcome_unknown`, `job_not_found`, `fenced_before_completion`, `requeued_for_retry`, `processing_error`, `failed`, `fenced_after_completion`, `completion_not_recorded`, `completed`), `disposition` = `ack` / `reject` / `requeue` | How `fix-worker-dependency-outage-disposition`'s decision table actually behaves, measured rather than argued: **sum by `(disposition)`** for the dead-letter rate, **group by `(outcome)`** to attribute it to a cause without reading logs. |
| `fiapx_worker_recovery_sweep_actions_total` | `worker` | `action` = `requeued` / `abandoned_no_source` / `abandoned_exhausted` / `requeue_refused` / `requeue_error` / `requeue_raced` | How often the sweep confirms a job unleased and what it then does about it. A rising `abandoned_exhausted` means jobs are burning through all `domain.MaxJobRequeues` (3) attempts rather than finishing on the first requeue. |
| `fiapx_worker_recovery_sweep_requeue_epoch` | `worker` | none | The `lease_epoch` a job held right after a successful requeue — 1, 2 or 3 at the default bound. A distribution skewed toward 3 means a fleet is close to exhausting its requeues before it starts failing jobs outright. |
| `fiapx_notifier_event_delivery_duration_seconds` | `notifier` | `disposition` = `handled` / `deferred` | How long one terminal event's whole run through `DeliverNotification.Execute` takes — claiming, attempting and resolving every deliverable preference. Compare its high percentiles against the next row. |
| `fiapx_notifier_delivery_max_claim_hold_seconds` | `notifier` | none | The configured `DeliveryConfig.MaxClaimHold()`, published once at startup, so a dashboard can overlay it on the histogram above without reproducing that arithmetic by hand — the only way to see the notifier's own drain budget being approached before a run actually exceeds it. |
| `go_*`, `process_*` | all five | fixed | Goroutine leaks, heap growth, GC pressure, descriptor exhaustion. |
...
- **An age is clamped at zero.** `created_at` and `occurred_at` are minted by PostgreSQL itself (`mint-videojob-timestamps-in-database`), read from the writing transaction's own `now()`, so the subtraction compares PostgreSQL against PostgreSQL rather than a writer's clock against the database's. The clamp is now a defensive bound rather than a correction for skew: nothing in ordinary operation should produce a negative value, but a negative age is still not a value this gauge can report.
- **They are reported per replica.** They describe shared database state, so each `video-api` replica reports the same fact. Aggregate with `max`, never `sum`, and keep each group's own label — `max by (state)` for the two job gauges and `max by (event_type)` for the two outbox gauges, since dropping `event_type` collapses the three relay backlogs into one and hides which relay stopped — with one replica today this is invisible, and scaling the service would silently multiply a summed queue depth.
...
#### What these metrics still do not see

Health and readiness left `cmd/worker` and `cmd/notifier` with no liveness signal, and that has not changed: neither process gained a probe, and `expose-worker-and-notifier-metrics` did not add one — a metrics-only listener is not a liveness signal, and giving either process a port for that purpose is explicitly the thing this row declined to do (see `docs/roadmap.md`). What it closed is the gap `add-prometheus-metrics` could only narrow from outside:

- **Visible from `video-api`'s own pipeline gauges, as before:** a worker that stopped consuming (the oldest-`queued` age climbs), a worker stuck on a job (the oldest-`processing` age climbs past any extraction's length), and a stopped terminal relay inside the worker (the oldest unpublished `video_job.completed.v1` / `.failed.v1` age climbs).
- **Now visible directly, from inside the two processes themselves:** how long an extraction takes and whether a slow one is a slow success or a slow failure (`fiapx_video_extraction_duration_seconds`); which branch a dispatch took — completed, failed, fenced, dead-lettered, requeued — and whether the consumer acked, rejected or requeued it (`fiapx_worker_dispatch_outcomes_total`); how often the recovery sweeper requeues a job versus abandons it, and which of its three chances a requeued job had just spent (`fiapx_worker_recovery_sweep_actions_total`, `fiapx_worker_recovery_sweep_requeue_epoch`); and how close a notification delivery run comes to its own `MaxClaimHold()` budget before the notifier's shutdown drain would have to give up on it (`fiapx_notifier_event_delivery_duration_seconds` against `fiapx_notifier_delivery_max_claim_hold_seconds`).
- **Still not covered by `video-api`'s own gauges:** the notifier's delivery records live in the Notification database, which `video-api` holds no pool for. That gap is now closed a different way — directly, by the notifier's own two families above — rather than by extending the video pipeline gauges to a database they were never going to read.
- **Still genuinely absent, and unchanged by this row:** a liveness signal for either process, and any record on an idle-but-wedged stack — the sweeper still logs only when it finds something, and the notifier's records are still all per-message. An idle-but-wedged worker or notifier is still indistinguishable from an idle-and-healthy one purely from inside either process; what an operator now has that they did not before is a dispatch-outcome rate and a sweep-action rate that go quiet in exactly the same way a healthy idle stack does, so "quiet" alone still is not the signal — a quiet rate *alongside* a climbing pipeline-gauge age is.
