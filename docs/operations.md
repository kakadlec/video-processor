...
An SMTP adapter behind the same `Deliverer` port the webhook channel already uses, plus a mail service in `docker-compose.yml` for local development. Until it ships, `email` is a **rejected** `Channel` value on `PUT /api/notification-preferences` — a channel a user can configure and that silently delivers nothing is worse than a rejected value. Notification never calls Identity, and does not need to: the address is the preference's own `Destination` — the column the webhook channel stores its URL in — supplied by the user on `PUT /api/notification-preferences`. It is therefore self-declared rather than the verified address Identity holds, which is bounded but not eliminated by the fact that a delivery is only ever triggered by a job that same user owns.
