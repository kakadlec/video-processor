...
video-processor/
  cmd/
    api/
      ...
      notification.go   # Notification composition root + handlers: wires internal/notification into main.go's router, handles GET/PUT /api/notification-preferences
      ...
      notification_test.go # Preference route/owner-scoping/secret-non-disclosure tests, plus the cross-context event-type constant pin
      web/
  ...
  internal/
    ...
    notification/
      ...
      infrastructure/ # Own PostgreSQL adapter (preferences + deliveries, advisory-locked Migrate),
                      #   own copy of the terminal topology/messages + Consumer, webhook Deliverer (payload, HMAC signature, guarded dialer)
    platform/
  ...
  Dockerfile       # Multi-stage build: builder -> test -> runtime (non-root); builds and carries all three binaries
  docker-compose.yml # Local dev stack: postgres, redis, minio, rabbitmq, app, worker, notifier, and the app-test service used to run the suite
  .github/
    workflows/
...
Phase 7's `add-notification-webhook-delivery` gave the terminal-event queue its consumer. The notifier is a third process, from the same image, with the narrowest configuration surface of the three: the Notification context's own DSN and the broker URL. It listens on no port, authenticates no caller, stores no artifact, holds no lease, and runs no `ffmpeg`.
...
video.jobs.terminal.events.v1        cmd/notifier/main.go          internal/notification/application
  ...
  │   │     └─ guarded dial ─► HMAC-signed POST ─► the registered destination
...
**The destination policy is applied at two points, and neither is redundant.** `SetPreference` judges the destination when it is registered; the delivery client's `net.Dialer.Control` judges the **resolved address** when a connection is opened. A write-time check cannot survive a hostname that resolves elsewhere later, nor a policy tightened after the row was stored; a dial-time check alone silently accepts a destination that will never be delivered to, which to its owner is indistinguishable from one that works. Both halves read one variable through one parser, so the two processes cannot disagree. The address rule is a permission — only globally reachable unicast — expressed as an explicit prefix table *in addition to* the standard-library predicates, since `IsGlobalUnicast()` answers yes for shared, benchmarking, and reserved space. The transport's `Proxy` is `nil` explicitly: with a proxy in the environment the guarded dial would approve the proxy's address and the proxy would resolve the user's hostname itself, bypassing the whole table.
...
| **Notification** | Delivery preferences — the `NotificationPreference` aggregate, its storage, and its authenticated owner-scoped HTTP surface — plus consuming a job's terminal outcome and delivering it to the destinations its owner registered | Implemented for the webhook channel (Phase 7, `add-notification-domain-and-preferences` + `add-notification-webhook-delivery`): preferences are stored and manageable, `cmd/notifier` consumes the terminal-event queue, and delivery is claimed, signed, bounded, and recorded. **`email` is still rejected** until `add-notification-email-delivery` ships an adapter, and `UserRegistered` is still unconsumed (`add-identity-user-registered-event`). Being reachable over HTTP does not make it a context others call to trigger a delivery |
...
video-processor/
  ...
  internal/
    ...
    notification/                    # Implemented for the webhook channel (Phase 7 — add-notification-domain-and-preferences + add-notification-webhook-delivery), wired into cmd/api and cmd/notifier
      ...
      infrastructure/
        postgres/     # This context's own Config/Open/Migrate/PreferenceRepository/DeliveryRepository over NOTIFICATION_POSTGRES_DSN and its own notification_preferences and notification_deliveries tables. Migrate holds a pg_advisory_xact_lock covering both CREATE TABLE IF NOT EXISTS statements, deliberately stricter than the identity and video adapters. Set is one atomic statement on either branch and reads no row; ClaimDelivery is likewise one atomic INSERT ... ON CONFLICT that both inserts and refuses; every read projects has_secret and never selects the secret except FindDeliverable, the single named delivery-path read
        messaging/    # This context's OWN copy of the terminal topology and the two message structs — copied rather than imported, because the cross-context ban is a property of the build — plus a Consumer whose disposition set is Ack/Reject/Requeue. Both copies are pinned field by field by tests in cmd/api, the one place allowed to import both contexts
        webhook/      # The Deliverer: the envelope (its own top-level version, independent of the event type's .v1), HMAC-SHA256 signing over timestamp and body, an http.Client whose dialer Control applies the destination policy to the resolved address, an explicitly nil Proxy, no redirects, a capped response body, and the shared LoadDestinationPolicyFromEnv parser cmd/api reads too
        # Email adapter: add-notification-email-delivery
```
...
|---|---|---|
| PostgreSQL | Authoritative state store for users, plus `video_jobs`/`video_job_outbox` (Phase 3) and `notification_preferences`/`notification_deliveries` (Phase 7) | **Implemented** (Phase 2 for identity; Phase 3 schema/adapter for video, wired into `cmd/api` by `wire-videojob-http-endpoints` and driven by `POST /upload` since `migrate-ffmpeg-execution-to-videojob-application`; Phase 7 for notification, whose pool is opened and migrated by `cmd/api` **and** `cmd/notifier` alike), required at deployment time — see [docs/operations.md](operations.md) |
...
4. `cmd/api`, `cmd/worker`, and `cmd/notifier` are the only places where `infrastructure` adapters are instantiated and wired (composition root), and all three now exist. `cmd/api/identity.go` plays that role for Identity, `cmd/api/video.go` for the use cases the HTTP surface needs (`CreateVideoJob`/`GetJobStatus`/`ListUserJobs`/`EnqueueVideoJob`), and `cmd/api/notification.go` for Notification's write side (`SetPreference`/`ListPreferences`, plus the write-time half of the destination policy); `cmd/worker/main.go` wires the ones the pipeline needs (`ProcessVideoJob`, which drives `StartProcessing`/`FailJob`, plus `CompleteJob` and `ClearJobIdempotencyKey`); `cmd/notifier/main.go` wires `DeliverNotification` and the dial-time half of the same policy. No binary switches behaviour on a mode flag, and each requires only the configuration it uses. A composition root MAY import more than one context — that is what makes it one — which is why the cross-context pins live in `cmd/api`.
