...
| **Notification** | Delivery preferences — the `NotificationPreference` aggregate, its storage, and its authenticated owner-scoped HTTP surface — and, later, reacting to domain events and delivering notifications (email, webhook) | Partially implemented (Phase 7, `add-notification-domain-and-preferences`): preferences are stored and manageable; **no consumer and no delivery yet** (`add-notification-webhook-delivery`, `add-notification-email-delivery`). Being reachable over HTTP does not make it a context others call to trigger a delivery |
...
video-processor/
  ...
  internal/
    ...
    notification/                    # Partially implemented (Phase 7 — add-notification-domain-and-preferences), wired into cmd/api
      dependency_rules_test.go  # Fails the build on an import of internal/video or internal/identity — the cross-context rule this context is built under
      domain/         # NotificationPreference (write intent / stored preference / read view, three shapes on purpose), its own UserID, the closed EventType and Channel sets, Destination, the non-disclosing Secret, and the PreferenceRepository port (implemented). DeliveryAttempt: Phase 7's delivery changes
      application/    # Use cases: SetPreference, ListPreferences (implemented). SendJobCompletionNotification, …: Phase 7's delivery changes
      infrastructure/
        postgres/     # This context's own Config/Open/Migrate/PreferenceRepository over NOTIFICATION_POSTGRES_DSN and its own notification_preferences table (implemented). Migrate holds a pg_advisory_xact_lock, deliberately stricter than the identity and video adapters. Set is one atomic statement on either branch and reads no row; every read projects has_secret and never selects the secret
        # Email adapter, webhook adapter: Phase 7's delivery changes
```
...
Rules 1–3 for `internal/identity/{domain,application}`, `internal/video/{domain,application}`, and `internal/notification/{domain,application}` are each enforced by an automated test (`internal/identity/dependency_rules_test.go`, `internal/video/dependency_rules_test.go`, `internal/notification/dependency_rules_test.go`), not just convention. Notification's adds rule 5 to what it checks: `internal/video` and `internal/identity` are forbidden import prefixes there, which is why the context declares its own `UserID` and its own copies of the `video_job.completed.v1`/`video_job.failed.v1` strings rather than importing either. That duplication is pinned at the one place allowed to see both sides — `TestNotificationEventTypesMatchTheEmittedTerminalEventTypes` in `cmd/api`, the composition root, in the same spirit as `TestRoutingKeyMatchesTheOutboxEventType`.
