...
**Status cache (Phase 4, `add-videojob-status-cache`):** `internal/video/infrastructure/cache.CachedVideoJobRepository` decorates the PostgreSQL `VideoJobRepository` with a Redis-backed cache for `FindByID` lookups, wired into `cmd/api/video.go`'s `setupVideo` so every use case that touches a `VideoJob` — including `GetJobStatus`, which backs `GET /api/video-jobs/:id`'s repeated polling reads — shares one cached repository instance. Reads are cache-aside (a hit skips PostgreSQL; a miss, Redis error, or corrupted entry falls back to PostgreSQL and repopulates the cache); the four state-transition use cases' writes are write-through (`Update` writes PostgreSQL first, then overwrites the cache entry with the confirmed new state). Miss-repopulation uses `SET NX`, not a plain `SET`, so a slow reader that read a pre-transition value can never clobber a fresher write-through entry a concurrent transition already wrote — a race caught during review and covered by a dedicated concurrency test. A corrupted cache entry is removed via a compare-and-delete Lua script (mirroring the idempotency store's own atomicity pattern) before repopulation, so it self-heals without reopening that same race. Entries carry a fixed 5-minute TTL as a safety net, not the source of correctness. `FindByUserID` (the `GET /api/video-jobs` list endpoint) and `Create` are intentionally left uncached. This closes out all three of Phase 4's planned Redis responsibilities. See `openspec/specs/videojob-status-cache/spec.md` for the full contract.
