...
The fourth Redis responsibility is implemented as a **lease for liveness**, not a lock around pickup. Concurrent pickup remains prevented by PostgreSQL's literal `status = 'queued'` claim. Redis tells the sweeper whether a `processing` row still has a live worker; PostgreSQL's `lease_epoch` and conditional terminal update provide the fence, so Redis failure cannot authorize two owners or let a superseded worker overwrite its successor.
