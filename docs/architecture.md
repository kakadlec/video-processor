...
video.jobs.queued.v2  (prefetch 1, one delivery at a time)
  ...
  └─► handle(body)
        ...
        │    ├─ FailJob if extraction failed, OR fetch/store failed with
        │    │    the source genuinely absent (ErrSourceNotFound)
        │    │    (processing → failed), conditional on the claimed epoch
        │    └─ RetryVideoJob if fetch or store failed for any other
        │         reason (processing → queued, epoch+1, fresh outbox row),
        │         conditional on the claimed epoch, bounded by
        │         MaxJobRequeues and paced by storageRetryPause — see
        │         "Transient object-storage retry" below
        ...
        ├─ ErrJobFenced from failure, completion, or a refused retry-requeue:
        │    └─ Reject → DLQ; keep source and idempotency key; release no
        │         lease; log held epoch and any result key
        ├─ ErrJobRequeuedForRetry (the RetryVideoJob write above applied):
        │    ├─ Release the lease at this run's held epoch (gated on Applied)
        │    ├─ Keep the source object and idempotency key — the fresh
        │    │    dispatch the write already committed still needs both
        │    └─ Ack (this delivery's job is done; a new one is coming)
...
**Transient object-storage retry (`retry-transient-object-storage-failure`).** Before this change a failed `SourceStorage.Get` or `ResultStorage.Put` reached `ProcessVideoJob`'s own `failWith`, which committed a terminal `failed` before the worker's `handle` ever got a chance to choose a disposition — so a momentary MinIO blip permanently failed every job that touched it, with no message-level retry able to undo an already-committed terminal state. The fix moves the decision earlier: `ProcessVideoJob.retryOrFail` intercepts a `Get`/`Put` error before any terminal write, and for every cause but a genuinely missing object (`domain.ErrSourceNotFound`, which no retry can produce) it calls `RetryVideoJob` — a new application use case that loads the job, calls `VideoJob.Requeue()`, and persists through `VideoJobRepository.Requeue`, the *exact same* conditional `processing → queued` statement, epoch fence, and fresh-outbox-row write the recovery sweeper's own `Requeue` call above uses. The two callers share one bound, `domain.MaxJobRequeues` (3), for the same reason they share the statement: both act on one job's `lease_epoch`, and letting one caller grant more attempts than the other would make "how many chances does this job have left" depend on which caller last touched it. `retryOrFail` pauses `storageRetryPause` (5s — the same duration as `messaging.DefaultRequeuePause`, duplicated rather than imported because the application layer does not depend on infrastructure) before writing, so the object store gets a moment before the next attempt's dispatch is even published. A success returns `domain.ErrJobRequeuedForRetry`, which `handle` acks without touching the source object or idempotency key — the fresh `video_job.queued.v2` outbox row the write already committed is a new, independent dispatch, so this delivery's job is done whether or not this particular message is what finishes the row. There is deliberately no fourth `videomessaging.Disposition`: unlike `ErrJobClaimOutcomeUnknown`, where the worker genuinely does not know what happened, here the worker already acted — the row is already back in `queued` by the time `handle` sees the sentinel. Exhausting the bound, or a `Get` that fails because the object is simply not there, falls back to the ordinary `failWith` and its ack-with-cleanup. This covers a momentary blip, not a sustained outage: a MinIO outage that outlasts three attempts (each paced ~5s apart, plus the outbox relay's own poll interval and whatever worker capacity picks up the fresh dispatch) still exhausts the bound and permanently fails the job — the same shape a PostgreSQL outage after the claim already has via the sweeper, just faster to exhaust because it is counted rather than paced indefinitely like `ErrJobClaimOutcomeUnknown`'s own retry.
...
Phase 7's `add-notification-webhook-delivery` gave the terminal-event queue its consumer, and `add-notification-email-delivery` gave it a second channel. The notifier is one of the five processes built from the same image, and the only one off both the request path and the extraction path. Its configuration is the Notification context's own DSN, the broker URL, and what each delivery channel needs to send: nothing for `webhook`, which dials the URL the user registered, and a relay address and envelope sender for `email`, which dials neither. It listens on no port, authenticates no caller, stores no artifact, holds no lease, and runs no `ffmpeg`.
...
Two absences carry weight. **Only `cmd/identity-api` is given a private key**, and the other two HTTP services hold no code path that could construct an issuer even if one were handed to them — `NewVerifier` refuses a private-key PEM outright. And `cmd/worker` reads **no `IDENTITY_*` and no `NOTIFICATION_*` at all**: it makes no access-control decision and registers no preference, so requiring either would misrepresent what the process does. Neither it nor `cmd/notifier` serves any *application* HTTP — no probe, no route a caller or the gateway ever reaches — and since `add-health-and-readiness-endpoints` that is pinned: each carries an in-package source test over its own non-test files asserting that the package constructs no HTTP server and imports no HTTP framework, strictly stronger than `container-image`'s "expose no port at all", which a process could satisfy while listening on one. `expose-worker-and-notifier-metrics` narrowed that test rather than dropped it: `cmd/worker/metricsserver.go` and `cmd/notifier/metricsserver.go` are each named as the one file, and `net/http.Server`/`NewServeMux` as the one construction, still permitted, so a second listener anywhere else in either package still fails it. What it builds is a `GET /metrics` handler alone, bound to `:9102` (worker) or `:9103` (notifier) and reachable only from inside the compose network — the gateway never proxies it and `docker-compose.yml` never publishes it to the host — never given a probe, a bearer group or a limiter, since neither process authenticates a caller or consults a readiness dependency. A raw `net.Listen` is still the stated residual neither test catches. `RABBITMQ_URL` is required wherever it appears, though a *reachable* broker never is. See [docs/operations.md](operations.md) for the full table.
...
#### Probe endpoints (deliberately not in the table above)

Each of the three HTTP services also serves `GET /health` and `GET /ready`, and they are kept out of the route table because a probe is **not a route of the application**: it serves no user-facing behaviour, appears in no documented flow, is consumed by the runtime, and — unlike every row above — is not reachable through the gateway at all. The gateway refuses both paths with two exact-match `location = … { return 404; }` blocks carrying no `proxy_pass`, so the refused request reaches no service. Without them the result would have been asymmetric rather than chosen, since the gateway sends everything it does not otherwise name to `cmd/video-api`: that one service's probes would have been public while the identical paths on the other two were not.

`GET /health` consults nothing and always answers `200 {"status":"ok"}`. `GET /ready` consults that service's readiness dependencies on every request and answers `200 {"status":"ready"}` or `503 {"status":"not ready"}`. Both bodies are fixed per verdict, written with `c.Data` rather than `c.JSON` so that what goes on the wire is the literal and cannot acquire a field when someone extends a struct, and both carry `Cache-Control: no-store`. Neither names the dependency that failed — the status code carries the whole answer, and a body that varied per cause would publish this deployment's dependency inventory to an unauthenticated caller, the same posture as the destination policy's single sentinel and the download route's byte-identical `404`.

**The two differ in criterion, not only in path, and that is the whole reason there are two.** Restart is the remedy a runtime wires to a liveness failure, and restarting a process cannot repair a dependency: a single endpoint that both consults dependencies and drives restarts turns a bounded outage into a crash loop that outlives it, because every restarted replica re-runs a startup this repository specifies as fatal on exactly that dependency.

**The readiness matrix is per service** — `cmd/identity-api`: PostgreSQL. `cmd/video-api`: PostgreSQL and MinIO. `cmd/notification-api`: PostgreSQL. Redis and RabbitMQ are consulted by none of them, and the criterion that produces those answers was **cited rather than invented**: it is the one `docker-compose.yml` already applies to the gateway's own healthcheck, which asks only whether nginx's configuration is loadable because *the gateway is healthy when it can serve, and tying its health to a backend's would make an unrelated service's restart look like an ingress failure*. Generalized — a readiness dependency is one whose absence stops the process serving — Redis falls out of `cmd/video-api`'s and `cmd/notification-api`'s matrices because every feature over it fails open (`cmd/identity-api` holds no Redis client at all, so there is nothing there to exclude), and the broker falls out because `POST /upload` commits the `queued` transition and its outbox row in one PostgreSQL transaction and answers `202` with the broker down. Structurally, too — and the wording has to be exact, because `cmd/video-api` **does** hold an AMQP connection whenever its dispatch relay is serving: the relay opens one inside its own dial cycle and closes it at the end of that cycle, so no composition-root handle and no probe handle exposes it. What the readiness path lacks is *access* to a relay-owned connection, not a connection somewhere in the process. And `internal/platform/rabbitmq`'s health check takes a live `*amqp.Connection` and no context, so it could not be bounded as every readiness check is even if one were reachable.

Both endpoints are registered on the engine, **outside** `requireBearerAuth()` and outside `rateLimitMiddleware`. This does not weaken the invariant stated for `setupRouter` above, which is the middleware *pair and its order* rather than the claim that every route sits inside the group: the limiter keys on the authenticated subject, a probe carries none, and a probe at a fixed interval from inside the limiter would eventually be answered `429` — the limiter manufacturing the outage it is meant to have no part in.

The object-storage check asks whether the **configured bucket is present**, which the adapter's existing `Ping` deliberately does not: `Ping` calls `BucketExists` and discards the boolean, because startup pings *before* `EnsureBucket` creates the bucket and a presence-asserting `Ping` would make a first start against an empty object store fatal on a bucket the next line was about to create. Two callers, two questions, two operations — `storage.CheckBucket` is the new read-only one, and it creates nothing.

Each check is bounded at 2s derived from the request context, and the checks run concurrently where a service has two, so the worst case is the slower check rather than the sum. **That bound must stay strictly below the timeout of whatever probes the endpoint** (5s on each compose healthcheck): reversed, the prober abandons every request and every verdict becomes a failure whatever the dependency is doing — the signal inverts rather than degrades, and inverts without emitting anything that says so.
...
- A "Notificações por e-mail" section, shown only while signed in, that subscribes the user to e-mail on a failed and/or completed job (`add-notification-email-preference-ui`). It writes only what the user selected — an unchecked box with no stored preference sends nothing, a deselected stored one is written disabled — uses one address field for both event types and warns when the two stored addresses differ, and ignores any response that arrives after the session that started it has changed. Webhook preferences are not managed by the page
- Vanilla JavaScript in `cmd/video-api/web/app.js` using `fetch` to call `POST /upload`, `GET /api/video-jobs/:id`, `GET /api/status`, `POST /api/auth/register`, `POST /api/auth/login`, and `GET`/`PUT /api/notification-preferences`; the bearer token is kept in `localStorage` and attached as an `Authorization` header on protected requests
...
Every process emits JSON records through `log/slog`, on standard output, at one format that no configuration selects. `internal/platform/logging` holds the whole of the mechanism — `New(service, level)` builds the JSON handler and binds the two identity fields, `ParseLevel` reads `LOG_LEVEL` and **refuses** a value it does not recognize rather than falling back, and the canonical service set (`identity-api`, `video-api`, `notification-api`, `worker`, `notifier`) is a set of constants rather than five string literals. It imports no HTTP framework and no bounded context, and a placement test in the package enforces that, as `internal/platform/rabbitmq` does.

**Each composition root builds the logger and installs it as the process-wide default, as the first thing it does.** That ordering is the point rather than tidiness: the severity is the one setting that has to be read before the logger it configures exists, so every *other* configuration failure — a missing DSN, an unbuildable verifier, a refused key — is already reportable as a structured record by the time it can happen. A `LOG_LEVEL` that will not parse is itself reported through a bootstrap logger carrying the same `service` and `instance` fields, and the process exits non-zero; there is no unstructured escape hatch anywhere.

**`internal/` packages reach the logger through `slog.Default()`, not through injection.** This is a deliberate departure from how every other dependency in this system is wired, and its cost is named rather than hidden: a package that logs through the default is coupled to process-global state, and a test that reads what it writes has to swap that global and cannot run in parallel. What injection would cost instead is an argument on constructors that need one for no other reason, and a bounded context acquiring an opinion about its process's logging — which is the thing the composition roots exist to hold. The one package that already accepted a logger so its own tests could read it, `internal/notification/application`, keeps doing so; its parameter is now a `*slog.Logger` and nil still means the default. Injection was not introduced anywhere it was not already present.

**Three rules are enforced against the source rather than against behaviour**, by one AST walk over every non-test file under `cmd/` and `internal/` (`internal/platform/logging/disclosure_test.go`):

1. **Typed attributes only.** Every argument after a call's message — and every argument to `Logger.With`, whose arguments are all attributes and which a message-relative rule does not reach at all — must be `slog.String`, `slog.Int`, `slog.Int64`, `slog.Bool`, `slog.Duration` or `slog.Time`. `slog.Any`, `slog.Group` and the loosely-typed alternating key/value form are refused. The rule is stronger than redacting known-sensitive types on purpose: a JSON encoder walks a value's exported fields with no opportunity for the type to intervene, so a domain value carrying a user-supplied connection target would be written out verbatim, query string included. Foreclosing the arbitrary-value path covers the types nobody has thought to defend yet. `WithGroup(name)` binds no value and is deliberately not covered.
2. **No unstructured output path.** No import of the standard `log` package at all — an import ban rather than a list of function names, because six of the migrated calls were methods on a `*log.Logger` instance that no enumeration of `log.Print`/`log.Fatal` reaches — and no `fmt.Print*` or `fmt.Fprint*` whose destination *resolves* to a standard stream or a framework default writer. The walk traces an identifier back to its assignment to decide that, because `os.Stdout` is an `*os.File` like any other and a type check cannot tell them apart; `internal/notification/infrastructure/smtp` writes the mail message with eleven legitimate `fmt.Fprintf` calls into a `strings.Builder`, and a name-based check would reject every one.
3. **Every message is a string literal.** A `fmt.Sprintf`ed message satisfies both rules above and the migration's own call-site count while putting the identifier straight back inside the message — the exact defect the change exists to remove, landed by the change itself.

A behavioural test cannot hold any of the three: it observes only the call sites that existed when it was written.

**`gin.Default()` is gone from all three HTTP roots**, replaced by `gin.New()` plus a per-root `logging.go` carrying the access-log middleware and a hand-written recovery handler, with `gin.SetMode(gin.ReleaseMode)` called explicitly in each `main()` before `setupRouter` — route registration itself prints `[GIN-debug]` lines, so the mode has to be set before it, and pinning it means `GIN_MODE` no longer decides anything. The recovery handler is written out rather than delegated to `gin.CustomRecovery` for two reasons found in the pinned v1.12.0: it prints an ANSI block to its own writer *before* calling the supplied handler, and passing a nil writer to silence that also skips the handler entirely on the broken-pipe branch (`EPIPE`, `ECONNRESET`, `http.ErrAbortHandler`), which would leave that one class of panic recorded nowhere. The middleware keeps gin's branch structure and each branch's response, and records both.
...
**One class of request is exempt from the access record — a request that matched `GET /health` or `GET /ready` — and the exemption is a closed list of those two templates with no general mechanism behind it.** That distinction matters more than the exemption: a configurable skip list would let a future route stop being recorded without that ever being reviewed. What it trades away carries no information anyway — a liveness record is `200` at near-zero duration on every occurrence, varying in no field, and probes arrive at a fixed interval forever across three services. What replaces it is **one record per change of readiness verdict** (`component: readiness_probe`), `warn` on degradation naming the failing dependencies, `info` on recovery, swapped by an atomic compare-and-set so two probers observing one transition produce one record. A per-request record that says nothing becomes a per-transition record that says more — including which dependency failed, which the response body is forbidden to say and an access record could not have said at all. The exemption does not reach the recovery record: a panic while serving a probe route is recorded like any other, because what is exempt is the routine per-request record and not the report of a failure.

The dependency names are joined into one comma-separated `slog.String` rather than passed as a slice, which is rule 1 above doing its work: `slog.Any` is denied by name, so there is no way to hand the logger a `[]string`. They come from a closed set each root owns (`postgres`, `object_storage`) and never from anything a caller supplied.
...
The three HTTP services serve `GET /metrics` in the Prometheus text format on the gin route table. Like the probes, the endpoint is **not in the route table above**: it is refused at the gateway by a third exact-match block, answered only on a service's own `:8080` inside the network, and registered on the engine outside `requireBearerAuth()` and `rateLimitMiddleware` for the probes' reasons. Unlike the probes it is **access-logged** — their exemption rests on a prober's fixed-interval volume, and until `add-local-metrics-scraper` nothing in this repository scraped. That change gave `docker-compose.yml` a `prometheus` service (15s interval, the three HTTP services at the time) and revisited the exemption with a real interval to reason about: at 15s across three targets the added volume is comparable to the probes', but extending the exemption is a source change to three `logging.go` files and their tests rather than a compose one, so `/metrics` stays access-logged — see `docs/roadmap.md`'s shipped note on that row for the full reasoning.

**`cmd/worker` and `cmd/notifier` now serve `GET /metrics` too, off a second listener with no route table at all.** `expose-worker-and-notifier-metrics` gave each a `net/http.ServeMux` carrying that one path, bound to `:9102` (worker) or `:9103` (notifier) inside the process's own container — never `:8080`, never registered with gin, and reachable only from inside the compose network, since neither the gateway nor `docker-compose.yml` exposes it further. It carries none of the three HTTP services' surrounding apparatus: no probe, because neither process has one; no access log, because neither process keeps one; no bearer group or limiter, because neither authenticates a caller. Three families live behind it that no HTTP service could ever record because no HTTP service runs the code that produces them: `fiapx_video_extraction_duration_seconds` (declared in `internal/video/infrastructure/ffmpeg`, the one package that calls `ExtractFrames`, labeled by outcome), `fiapx_worker_dispatch_outcomes_total` (`cmd/worker/metrics.go`, one label per branch of `handle()`'s decision table plus the disposition it reports to the consumer — ack, reject or requeue), and `fiapx_worker_recovery_sweep_actions_total`/`fiapx_worker_recovery_sweep_requeue_epoch` (the same file, recording what `cmd/worker/sweeper.go`'s `recover` decided and, on a successful requeue, which of `domain.MaxJobRequeues` chances the job had just spent). `cmd/notifier/metrics.go` adds two more: `fiapx_notifier_event_delivery_duration_seconds`, timing `DeliverNotification.Execute` by the disposition `handle()` reports, and `fiapx_notifier_delivery_max_claim_hold_seconds`, a gauge published once at startup from `DeliveryConfig.MaxClaimHold()` so the histogram's high percentiles can be read against the same budget `Validate()` already enforces. All six pass the same label rule the three HTTP services' families do — `internal/platform/metrics/disclosure_test.go` walks `cmd/` and `internal/` alike, not only the three roots that used to be the only callers.

**The mechanism is shared and the vocabulary is not.** `internal/platform/metrics` holds one explicit registry (never the client library's default), the exposition handler, the Go and process collectors, and two label constructors. It holds **no metric family belonging to a bounded context**, for the reason `internal/platform/rabbitmq` may hold no context name — and it carries its own placement test to prove it, since that test parses `.` and a new platform package is unenforced until it has one. Every family is declared in the package that records it and registers itself: the per-request families in a fourth per-root copy beside `logging.go`, `probes.go` and `readiness.go` (`metrics.go`, identical through its shared part), the limiter's counter in the two roots that mount one, the reservation counter in `cmd/video-api`, and the cache counter in `internal/video/infrastructure/cache` — which `cmd/worker` also links, so it increments a family nobody serves. That cost is accepted on the logging precedent: families reach the registry the way records reach `slog.Default()`, and no constructor is threaded a registry it needs for nothing else.

**The label rule is logging's, made stricter on one axis.** A record may carry `job_id` and `user_id`; a label may carry no identifier at all, because a record ages out and a label value is a series that persists. `internal/platform/metrics/disclosure_test.go` holds it against the source with three rules — label values are literals or the two constructors, names and label names are literals, the default registry is never reached — and the first is a **permission with a failing default**: every method on a value settled as a metric vector is judged except the three that take no label, a constant-metric constructor of unknown arity fails, every appearance of `prometheus.Labels` other than a composite literal fails, and type names resolve through the file's imports so an alias cannot walk past. A domain value becomes a label only through a `switch` whose every arm passes a literal.

**The two variable labels are bounded by the router.** `metrics.Route` and `metrics.Method` resolve against the route table each root reads with `engine.Routes()` as the **last statement of `setupRouter`** — gin walks its trees at the moment of the call, so binding earlier leaves a real route resolving to `<unmatched>` for the life of the process — and translates into `[]metrics.RouteEntry`, a representation of ours, because nothing under `internal/` imports gin. Resolution is fail-closed: unbound, everything is `<unmatched>` / `UNRECOGNIZED`. The series count is therefore a product the build fixes.

**The pipeline gauges live in the adapter, over the undecorated repository.** `postgres.Repository` gained `InFlightJobAggregate` and `UnpublishedOutboxAggregate`, and **deliberately not `domain.VideoJobRepository`**: widening the port would oblige the cache decorator and every test double to carry them, and would make it possible to build the collector on the decorator — a cached count is a count from a different moment. Because the decorator does not carry them, that is a compile error rather than a convention, and a reflection test pins both halves. `postgres.PipelineCollector` queries on scrape under a 2s bound, emits **no sample and an error** on failure and **a zero count with no age** on an empty state, and is registered only by `setupVideo`. Each state's predicate is a literal, one `UNION ALL` branch per member of the closed set, because a partial index is matched only when the planner can prove the predicate — and it cannot for a parameter. Two partial indexes serve the job half, `(created_at) WHERE status = 'queued'` and `(created_at) WHERE status = 'processing'`; the sweeper's `(id) WHERE status = 'processing'` index finds the same rows but cannot order them, so it does not.

---

## Target Architecture (Implemented — Phases 1–8 done)
...
|---|---|---|
| nginx gateway | The single ingress: one location rule per HTTP service, the only *application* process publishing a host port | **Implemented** (`split-api-by-bounded-context`) — stock upstream image plus a mounted configuration file, carrying no binary, key, or credential of ours |
