...
To skip the manual wiring entirely, use `docker compose up --build`, which starts all five services plus the gateway, a mail catcher, and a Prometheus server inside Docker with everything already configured — see "Docker Workflow" below. The mail catcher (`mail`) accepts every message and delivers none onward; its inbox is at <http://127.0.0.1:8025>. Prometheus (`prometheus`) scrapes `GET /metrics` on the three HTTP services every 15s; its UI is at <http://127.0.0.1:9090>. These are the second and third host ports the stack publishes — both reached directly rather than through the gateway, which serves the application's own surface and would make a route for either a development-only UI that exists in no deployment.
...
#
# The two other published ports are the mail catcher's inbox, at
...
# lands there instead of being sent anywhere real — and Prometheus's UI, at
# http://127.0.0.1:9090, scraping GET /metrics on the three HTTP services
# every 15s (docker/prometheus/prometheus.yml). cmd/worker and cmd/notifier
# serve nothing, so they are deliberately absent from that scrape config
# until expose-worker-and-notifier-metrics changes that. Both are
# development-only support services, so each publishes its own loopback-
# bound port rather than taking a route on the gateway; Prometheus's own data
# has no named volume (24h retention besides), for the same reason redis's
# does not — diagnostic and non-authoritative, so a recreate starting
# against an empty anonymous volume rather than the previous run's is a
# correctness non-event, not a loss.
#
...
> The `Dockerfile` is a multi-stage build: a `builder` stage compiles static binaries (dependencies resolved read-only from the committed `go.sum` — the build fails rather than silently patching it), a `test` stage adds `ffmpeg` on top of `builder` for running the suite (see `app-test` above), and the default `runtime` stage — the one every service and deployment uses — ships **all five** compiled binaries (`/app/identity-api`, `/app/video-api`, `/app/notification-api`, `/app/worker`, `/app/notifier`) plus `ffmpeg`, no Go toolchain or source tree, running as a non-root user (fixed UID 1000). `ffmpeg` is there for the worker rather than for any HTTP service or the notifier. Every service other than the Video API is the same image with its command overridden.
>
> **The `test` stage runs on the machine you build from, and that is load-bearing for this command.** The `builder` stage it derives from is pinned to `--platform=$BUILDPLATFORM` so the image can be cross-compiled for a second architecture without emulating the Go toolchain, and `test` inherits that pin. If it followed the *target* platform instead, the image would still be correct and every requirement about it would still hold — while `docker compose run --build --rm app-test` ran the suite under QEMU and became too slow to use. `openspec/specs/container-image/spec.md` states this as a requirement for that reason. The `notifier` service also carries `stop_grace_period: 120s`, longer than its shutdown drain — one claim hold per delivery channel plus a 30s grace, 90s at the documented budget — Compose's 10-second default would `SIGKILL` a delivery still in flight and leave a claim unresolved on every `docker compose stop`.
>
