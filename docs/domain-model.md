...
- The Video Processing context's own `UserID` validates format only (non-empty), not existence — existence is guaranteed by the API middleware that runs before the handler and mints the already-verified value Video Processing's `UserID` wraps.
