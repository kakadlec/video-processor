...
**Domain events consumed:** `VideoJobCompleted` and `VideoJobFailed`, over RabbitMQ, by `cmd/notifier` as of `add-notification-webhook-delivery`. `UserRegistered` is still planned (`add-identity-user-registered-event`). A consumed message is translated at the composition root into `TerminalEvent`, the context's own model of an outcome — event type, job id, its own `UserID`, occurred-at, and the outcome's fields — so nothing downstream of that translation knows a wire format exists.
