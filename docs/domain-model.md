...
**Domain events emitted:** none in this slice — `RegisterUser`/`AuthenticateUser` return synchronously and don't publish events. Domain events for Identity (e.g. `UserRegistered`) are deferred until a consumer needs them; introducing them speculatively was an explicit non-goal of Phase 2's design, and as of Phase 7 no consumer is planned — Notification takes the e-mail address from the user's own preference `Destination`, not from Identity (see `docs/roadmap.md`'s Phase 7 note).
...
**Responsibility:** Manage the full lifecycle of a video processing job — accepting an upload, tracking state transitions, running the async worker, and making the result available. This is the core domain of FIAP X. It spans two processes: `cmd/video-api` accepts and reports, `cmd/worker` executes.
...
**Responsibility:** Own the user's delivery preferences — where and how they want to be told a job ended — and react to domain events from Video Processing to deliver those notifications. Both channels are implemented end to end: webhook as of `add-notification-webhook-delivery`, e-mail as of `add-notification-email-delivery`.

**Aggregate root:** `NotificationPreference`, identified by the triple `(UserID, EventType, Channel)` — there is no surrogate id, because nothing references one by an id, and the triple gives the upsert its conflict target for free. Both `EventType` and `Channel` are **closed sets**, parsed through the domain and never re-checked in a handler: `video_job.completed.v1` and `video_job.failed.v1`, and `webhook` and `email`. The set stays closed at the channels an adapter actually delivers through, which is the same rule that kept `email` out of it until one existed — a channel a user can configure and that silently delivers nothing is worse than a rejected value. `AllChannels()` exposes the set so a composition root can be exhaustive over it rather than listing what it remembers; `cmd/notifier` refuses to start if any channel has no delivery implementation.

**A destination means whatever its channel needs, and the rule is per channel.** `NewDestinationFor(channel, raw)` picks it: a `webhook` destination is an absolute `http`/`https` URL with a host, an `email` destination is a single addr-spec address. One `destination` column stores both, because both are exactly one string and nothing queries inside it. The four sites that build a `Destination` — three reconstructions in the adapter and one in `SetPreference` — all hold the channel already, and the constructor's `default` branch refuses, so a `Channel` that was never parsed matches neither rule rather than falling through to whichever is written first.

**An e-mail address is validated as a contract, not as a formality, because header injection is the threat.** The rule is a byte-identical round trip through `net/mail` — which rejects display names, angle brackets, comments and quoted forms in one check rather than an enumeration of syntaxes — plus a printable-ASCII scan bounded at 254 bytes. The scan is the security half: a message is a header block, and an address carrying CR or LF that reached a header would let its registrant add recipients or a body of their own. It refuses CR, LF and NUL as a consequence rather than as three special cases, and refuses non-ASCII for a separate reason — `net/smtp` negotiates no SMTPUTF8, so such an address would be one the adapter could never put in an envelope.

The aggregate has **three shapes**, and the split is load-bearing rather than stylistic. A *write intent* carries an **optional** secret, because omitting one is a legitimate update. A *stored preference* carries one **when its channel signs**, because that create path refuses anything else. A *read view* carries `HasSecret bool` and no secret field at all. Collapsing them would force the use case to know create-from-update before writing — the pre-read the persistence design exists to avoid.
...
**A secret is required only where the channel signs, and that condition lives in four places rather than one.** HMAC signing is the webhook channel's mechanism; an e-mail message is not signed with a per-preference secret, so demanding one for it would store a credential-shaped field with no credential in it. `Channel.Signs()` names the property once, and the schema's `CHECK (channel <> 'webhook' OR secret <> '')`, the adapter's choice among three write statements, `RestoreNotificationPreference`'s invariant, and the delivery read's projection all condition on it. Three of those are obvious; the aggregate one is the one worth stating, because leaving it unconditional would make an e-mail row writable and permanently *unreadable* — and that failure arrives as a `FindDeliverable` error, which the consumer's disposition table classifies as *attempted nothing* and therefore **requeues**, so one such row would block the queue at prefetch 1 rather than fail visibly. `ErrSecretRequired` is unchanged for `webhook`.

**The webhook secret is a credential, and non-disclosure is its whole protection.** HMAC signing needs the original bytes, so it cannot be hashed the way a password is. It is accepted on write, never returned by any read, and never logged. **Exactly one** repository operation loads the value — `FindDeliverable`, the delivery path's named read, whose only consumer computes a signature with it — and even there the projection is conditional, yielding a secret only for a row whose channel signs. That last narrowing is not decoration: a preference on a non-signing channel *may* carry a secret, since a write submitting one stores it whatever the channel, and declining to *use* a value that was selected and scanned is a weaker guarantee than declining to *load* it. Every other read projects `secret <> ''` instead of the column. That is a narrowing of the rule rather than an exception carved out of it: the value has to be loadable somewhere for a signature to exist at all, and what keeps non-disclosure true is that the somewhere is singular, named, and provably absent from the HTTP composition root. Both halves are enforced by tests rather than by convention — a source-level assertion that no other query in the adapter names the column, and one that no file under `cmd/notification-api` calls the operation. `domain.Secret` renders a redacted placeholder from `String`/`GoString`/`Format`, refuses to `MarshalJSON`, holds its value behind an unexported pointer so `%p` reaches an address, and exposes the bytes through one deliberately-named `Reveal()`. Required on create, optional on update, and an explicitly empty value is rejected rather than read as a removal.

**Domain events consumed:** `VideoJobCompleted` and `VideoJobFailed`, over RabbitMQ, by `cmd/notifier` as of `add-notification-webhook-delivery`, and delivered on both channels as of `add-notification-email-delivery`. `UserRegistered` is not planned: the e-mail channel takes its address from the preference's own `Destination`, so Notification needs nothing from Identity. A consumed message is translated at the composition root into `TerminalEvent`, the context's own model of an outcome — event type, job id, its own `UserID`, occurred-at, and the outcome's fields — so nothing downstream of that translation knows a wire format exists.

One event resolves to at most one preference per channel, and each is claimed, attempted and resolved on its own: two records for one job, two independent outcomes, and a failure on either changes nothing about the other. The `TerminalEvent`'s own `JobID` is deliberately loose — it enforces only non-emptiness, because the value's shape is Video Processing's business — which is why the SMTP adapter re-checks every value bound for a message header before writing one. A malformed payload on the terminal queue is the shape of that attack, and refusing at the point the header is written is what avoids tightening a value object whose looseness is correct.
...
**Cross-context independence:** it imports neither `internal/video` nor `internal/identity` — enforced by `internal/notification/dependency_rules_test.go` — so it declares its own `UserID` value object and its own copies of the two event-type strings. `TestNotificationEventTypesMatchTheEmittedTerminalEventTypes` in `internal/contracts`, the one package that legitimately imports both sides, pins those copies against `internal/video/infrastructure/postgres`'s.

**Second aggregate: `Delivery`.** Identified by `(UserID, EventType, Channel, JobID)` — the preference triple plus the job whose outcome is being announced — with that uniqueness enforced by the database rather than by a read-then-write. It carries a status (`pending`, `delivered`, `failed`), an attempt count, a claimed-at and a resolved-at, a free-text reason written by this system, and **two identifiers that are not interchangeable**:

- **`DeliveryID`** is stable. It is what the receiver sees and deduplicates on, so it survives a reclaim — every attempt for one delivery, and every attempt after a reclaim, carries the same value.
- **`ClaimToken`** says *which grant is current* and is reissued on every grant, reclaims included.

**Claiming is one atomic statement that both inserts and refuses**, and reads no row first. Two consumers evaluating "has this been delivered?" separately would both read *no* and both deliver; the guarantee has to hold across processes, which a check-then-insert cannot give. The statement creates the record when none exists, refuses when one exists that is resolved or freshly claimed, and grants a claim over one left unresolved past the reclaim bound.

Refusal is **three-valued, not boolean**: `ClaimGranted`, `ClaimAlreadyResolved`, and `ClaimHeldByAnother`. Collapsing the last two is the bug — a resolved delivery means there is nothing left to do, while a held one may belong to a process that died before resolving, and treating that as handled would strand a `pending` row nothing else will ever meet. Classifying the refusal is a second, deliberately **non-authoritative** read: the grant decision stays one statement, and misreading the classification costs one extra pass through the handler, never a duplicate outbound request.

**Resolution is fenced on the claim token.** Expiry of the reclaim bound proves a claim is *old*, not that the process holding it stopped, so a slow first claimant can still be running when a second is granted. A write whose token is no longer current is refused and changes nothing — the same fence `VideoJob` puts on a terminal write with `lease_epoch`, for the same reason. That the bound must exceed one claimant's whole budget is checked at startup rather than left as a convention two independently-tuned values can break: the token fences the database write but cannot recall a request already on the wire.

The record holds no secret, no request body, and no part of the destination beyond what identifies the preference.

**What `delivered` means is channel-specific, and the record does not say which.** For a webhook it is the receiver's own `2xx`. For e-mail it is that the configured relay accepted the message: a bounce is asynchronous, is returned to the envelope sender, and is never observed by this system. Both are the strongest statement each channel can truthfully make, and inventing a third status would imply a bounce-processing capability that does not exist.

**Introduced in:** Phase 7 — `add-notification-domain-and-preferences` for the `NotificationPreference` aggregate, its PostgreSQL adapter, and the two routes; `add-notification-webhook-delivery` for `Delivery`, `TerminalEvent`, `DestinationPolicy`, and `cmd/notifier`; `add-notification-email-delivery` for the second channel, the per-channel destination rule, and the conditional secret invariant (see [docs/roadmap.md](roadmap.md)).
...
| `VideoJobCompleted` | `type`, `job_id`, `user_id`, `frame_count`, `storage_key`, `occurred_at` | Yes — Video Processing → Notification | **Recorded, published, and consumed** (Phase 7, `emit-videojob-terminal-events` then `add-notification-webhook-delivery`): `postgres.Repository.Update` writes this payload to `video_job_outbox` in the same transaction as the fenced terminal `UPDATE`, and only when that statement affected a row; `cmd/worker`'s terminal relay publishes it to `video.jobs.terminal.v1`. `type` and the routing key are both `video_job.completed.v1`. **Consumed** by `cmd/notifier` as of `add-notification-webhook-delivery`, which resolves it against the owner's preferences and delivers it as a signed webhook — under the Notification context's own payload contract, not this one forwarded |
| `VideoJobFailed` | `type`, `job_id`, `user_id`, `error_reason`, `occurred_at` | Yes — Video Processing → Notification | **Recorded, published, and consumed** (Phase 7, `emit-videojob-terminal-events` then `add-notification-webhook-delivery`): written and published exactly as `VideoJobCompleted` is, by the same transactional write and the same relay, under `video_job.failed.v1` onto the same queue, and consumed by the same handler — one queue carries both outcomes, since a consumer interested in a job's end is interested in either |
| `UserRegistered` | `type`, `user_id`, `email`, `occurred_at` | No — the crossing it would have served does not exist | **Not planned.** Deferred through Phase 2 until a consumer needed it, and Phase 7 decided none does: the e-mail channel reads its address from the preference's own user-supplied `Destination`, so no Identity outbox, relay or local projection is built. The field set above is what it would have carried |
| `UserAuthenticated` | `type`, `user_id`, `occurred_at` | No (internal) | Planned; not emitted by the current `AuthenticateUser` use case |

Integration events that cross context boundaries are published to RabbitMQ (Phase 6) via a transactional-outbox relay. That relay now exists (`openspec/specs/videojob-outbox-relay/spec.md`) and its source table is `video_job_outbox` (Phase 3), and there are now two of them, each claiming its own disjoint set of event types from that one table: the dispatch relay in `cmd/video-api` carries `VideoJobQueued` to the job queue that `cmd/worker` consumes, and the terminal relay in `cmd/worker` carries `VideoJobCompleted` and `VideoJobFailed` to the terminal-event queue. `VideoJobCreated` rows are written and deliberately never published — an internal event, and each relay's claim filters on `event_type` precisely so that permanent backlog cannot starve the rows that are meant to go out. The two events that cross to Notification are recorded as of `emit-videojob-terminal-events`: `Repository.Update` — the write path behind `CompleteJob` and `FailJob` — commits the event with the outcome, so a terminal job and its announcement cannot exist without one another, and an actor whose write was refused by the fence or found its own outcome already stored records nothing. `cmd/notifier` consumes them as of `add-notification-webhook-delivery`, and discharges the deduplication obligation with a durable delivery record claimed atomically before any request is made — not by trusting the transport. See `openspec/specs/videojob-persistence/spec.md` for what `video_job_outbox` currently records. Internal domain events do not need to be published to the broker. Identity (Phase 2) is implemented but does not emit either event above yet — see the Identity Context use-case table.

---

## Cross-Context Contracts

- **`UserID`** is the identifier that crosses bounded context boundaries, but not as a shared type: each context defines and owns its own local `UserID` value object (`internal/identity/domain/user_id.go` and, as of Phase 3's `add-videojob-domain-and-application`, `internal/video/domain/user_id.go`) — two distinct Go types, never a package shared between the two contexts' `domain` layers. Translation between a source context's identifier and a consuming context's local type happens only at the composition root or via consumed integration events. `cmd/video-api` is where that translation happens: `cmd/video-api/video.go`'s HTTP handlers (wired by Phase 3's `wire-videojob-http-endpoints`) convert the bearer-auth middleware's already-verified `identity.UserID` string into `video.UserID` on every request. `cmd/worker` (Phase 6) is a composition root of its own but performs **no** such translation — it never sees a token and reads the `user_id` off the dispatch message, which the API already translated. It has no Identity configuration at all. No `pkg/` directory exists or is planned — a shared kernel was considered and rejected (see `add-videojob-domain-and-application`'s `design.md` in the archive) as tighter coupling than this architecture's context-independence goal justifies.
...
- The Notification context receives integration events over RabbitMQ and resolves delivery preferences by `UserID` alone, never by calling into Identity or Video Processing internals. As of `add-notification-domain-and-preferences` those preferences are stored in its own database under its own DSN, and as of `add-notification-webhook-delivery` `cmd/notifier` resolves them against consumed events. The translation from the message's `user_id` to the context's own `UserID` happens in that composition root — the sanctioned crossing — and the context obtains the topology names and payload shapes it needs from its own declarations, never by importing the emitting context. Its independence is enforced rather than asserted — `internal/notification/dependency_rules_test.go` fails the build on an import of either other context.
