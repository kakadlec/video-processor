...
| `wire-videojob-http-endpoints` | New job-oriented endpoints backed by `VideoJob`, added alongside the legacy synchronous flow (not yet replacing it — this row only wires the new read/create paths). Note: until `migrate-ffmpeg-execution-to-videojob-application` below lands, a job created through these new endpoints has no way to leave `pending` (no `CompleteJob`/`FailJob` yet) — acceptable only because the frontend keeps using the legacy `/upload` flow throughout, so this doesn't affect real usage, but it should not be presented as a working end-to-end path until the next row ships. | `add-videojob-infrastructure`, `extract-cmd-api-entrypoint` | not-started |
