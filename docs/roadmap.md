...
Phase 3 is partially done: `internal/video/{domain,application,infrastructure}` is fully built and, as of `wire-videojob-http-endpoints`, wired into `cmd/api` behind a preview `/api/video-jobs` HTTP API (`VIDEO_POSTGRES_DSN` is now mandatory at startup too, alongside the `IDENTITY_*` variables). That API has no processing trigger yet — a job created through it stays `pending` forever until `migrate-ffmpeg-execution-to-videojob-application` (still not-started) migrates the legacy `POST /upload` handler's `ffmpeg` call into the application layer. The frontend keeps using `POST /upload` exclusively throughout this phase.
