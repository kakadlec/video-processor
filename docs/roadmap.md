...
| `fix-stale-pr-separation-rules-in-canonical-specs` | Two canonical specs are now stale relative to the prose PR-separation rule (`AGENTS.md`/`CLAUDE.md`/`docs/development.md`), which has been hand-edited twice without a corresponding spec update: (1) `openspec/specs/development-workflow/spec.md`'s "Implementation PR Contains Only Application Code and Tests" requirement still says the implementation PR "SHALL contain only... application source and test files", not reflecting the scoped config/CI exception added directly to the prose docs; (2) `openspec/specs/ddd-architecture/spec.md`'s "Documentation PR is isolated from spec and code PRs" scenario still describes documentation, archive, and roadmap-status updates as separate PRs, contradicting the now-combined "Finalization PR" role. Needs a proper change since both are canonical specs, not direct hand-edits. | none | not-started |
