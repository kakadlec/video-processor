...
| Monitoramento: Prometheus + Grafana, ELK ou similar | **Prometheus + Grafana.** Os cinco processos expõem `/metrics` — os três serviços HTTP na sua própria `:8080`, e worker e notifier num listener dedicado só para métricas (`:9102` e `:9103`), nenhum deles publicado no host nem roteado pelo gateway: quem alcança é o Prometheus, de dentro da rede. A pilha local sobe esse Prometheus raspando os cinco e um Grafana com dashboard provisionado a partir de arquivo (<http://127.0.0.1:3000>, sem login). Todos os processos emitem log estruturado JSON via `log/slog` |
