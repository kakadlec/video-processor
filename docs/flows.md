...
- **Notifications are delivered — by webhook.** The path is complete as of `add-notification-webhook-delivery`: `CompleteJob` and `FailJob` persist through `Repository.Update`, which commits the matching `video_job.completed.v1`/`video_job.failed.v1` outbox row in the same transaction as the fenced terminal `UPDATE` and only when that statement affected a row, so one job's outcome leaves exactly one record however many actors raced to finish it; a relay in `cmd/worker` publishes those rows to `video.jobs.terminal.v1`; and `cmd/notifier` — a third process from the same image — consumes the queue bound under both routing keys, resolves the job owner's registered preferences, and delivers each outcome as one HMAC-signed HTTP request. The relay's at-least-once delivery is deduplicated by a durable `notification_deliveries` record claimed atomically **before** any request is made, not by trusting the transport. A failed delivery changes nothing about the job: it is invisible to the Video Processing context and to every HTTP route. See [docs/architecture.md](architecture.md)'s "Delivery pipeline" for the flow and the disposition table.
- **What is actually still missing:** the **email channel** alone (`add-notification-email-delivery` — `email` is a rejected `Channel` value until an adapter honours it). Its address comes from the preference's own `Destination`, the same user-supplied column the webhook channel stores a URL in, so nothing reaches into Identity and no `UserRegistered` event is built.
