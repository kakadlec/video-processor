...
Every request below enters through the nginx gateway, which is the only *application* process publishing a host port (the local stack's mail catcher publishes its own inspection port, which serves no application route) and which routes by path prefix: `/api/auth/` to `cmd/identity-api`, `/api/notification-preferences` to `cmd/notification-api`, and everything else — including the frontend at `/` — to `cmd/video-api`. The gateway is omitted from the diagrams: it forwards the request unchanged, with the body unbuffered, and no flow below depends on it doing anything more.
...
## Notification delivery (Phase 7, complete)

- **Notifications are delivered, on both channels.** `CompleteJob` and `FailJob` persist through `Repository.Update`, which commits the matching `video_job.completed.v1`/`video_job.failed.v1` outbox row in the same transaction as the fenced terminal `UPDATE` and only when that statement affected a row, so one job's outcome leaves exactly one record however many actors raced to finish it; a relay in `cmd/worker` publishes those rows to `video.jobs.terminal.v1`; and `cmd/notifier` — one of the five processes built from the same image — consumes the queue bound under both routing keys, resolves the job owner's registered preferences, and delivers each one through the transport its channel names. The relay's at-least-once delivery is deduplicated by a durable `notification_deliveries` record claimed atomically **before** any request is made, not by trusting the transport. A failed delivery changes nothing about the job: it is invisible to the Video Processing context and to every HTTP route. See [docs/architecture.md](architecture.md)'s "Delivery pipeline" for the flow and the disposition table.
- **Two channels, two independent outcomes.** A `webhook` preference receives one HMAC-signed HTTP request; an `email` preference receives one plain-text message through the relay `cmd/notifier` is configured with. The triple allows one preference per channel per event type, so a single job can produce two delivery records, each claimed, attempted and resolved on its own — one channel failing changes nothing about the other. The routing is a `Deliverer` composed in the composition root, so the use case that holds the claim and the fence does not know a channel exists.
- **The e-mail address is the preference's own `Destination`**, the same user-supplied column the webhook channel stores a URL in, so nothing reaches into Identity and no `UserRegistered` event is built. It is therefore self-declared rather than the verified address Identity holds; what bounds that is that a delivery is only ever triggered by a job the same user owns. See [docs/operations.md](operations.md).
- **What is still missing:** nothing in Phase 7. Phase 8 is under way rather than pending: `add-structured-logging` shipped, so every process on this path — the API that accepted the job, the worker that ran it, and the notifier that announced it — emits JSON records naming the `job_id` as a field, and one filter reconstructs the whole path above from the collected stream. `add-health-and-readiness-endpoints` then shipped too, though it adds no step to the flow above: a probe is not a route of the application, appears in no flow, and is refused at the gateway. What remains of the phase is metrics alone, not yet decomposed — see [docs/roadmap.md](roadmap.md).
