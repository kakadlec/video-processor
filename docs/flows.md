...
- **What is actually still missing:** the **email channel** (`add-notification-email-delivery` — `email` is a rejected `Channel` value until an adapter honours it) and Identity's **`UserRegistered`** event (`add-identity-user-registered-event`), which email delivery needs since Notification never calls Identity.
