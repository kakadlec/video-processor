...
func newMetricsTestRouter(t *testing.T) *gin.Engine {
	t.Helper()

	return setupRouter(newTestIdentityModule(t), newChecker())
}
...
// This service mounts neither bearer authentication nor the limiter, so there
// is no group for the endpoint to be outside of, and the assertion is about
// registration rather than about a middleware being skipped. It is made here
// all the same: a scrape answered on two services and refused on the third
// would be an authentication rule nobody chose.
...
// This service mounts no limiter at all, so there is no budget test to write
// here: its two routes are how a caller obtains a token, and gating them
// would be circular. The corresponding assertion lives in the two roots that
// do mount one.
...
// TestNoLabelValueLooksLikeAnIdentifier drives the endpoint after traffic
// carrying an identifier in its path, so there is something in the process to
// leak, and then asserts none of it reached a label.
func TestNoLabelValueLooksLikeAnIdentifier(t *testing.T) {
	router := newMetricsTestRouter(t)

	serveRequest(t, router, http.MethodPost, "/api/auth/login", "")
	serveRequest(t, router, http.MethodGet, "/3fa85f64-5717-4562-b3fc-2c963f66afa6", "")
