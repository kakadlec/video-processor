...
//
// PostgreSQL is a readiness dependency because both preference routes are a
// single database round trip: with it unreachable, neither can be answered
// and nothing here is specified to degrade instead.
//
// The cache is deliberately absent even though main holds a client for it.
// This service reaches Redis for one thing, the per-user rate limiter, and
// that limiter is specified to fail open: a request arriving while Redis is
// down is allowed rather than refused. A service with the cache down is
// unmetered and still correct, so reporting not ready would remove a serving
// process from rotation for a condition its own specification calls
// survivable. The broker is absent because this process never opens one.
