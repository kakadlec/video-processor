...
// authenticator and notification modules. authUserID, if non-zero, is injected directly as
...
func TestRateLimitMiddleware_UnauthenticatedRequestNeverInvokesLimiter(t *testing.T) {
	...
	// the authenticated group, which never runs requireBearerAuth in the
	// first place).
