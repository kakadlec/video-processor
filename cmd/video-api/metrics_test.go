...
func newMetricsTestRouter(t *testing.T, limiter rateLimiter) (*gin.Engine, testTokens) {
	...
	return setupRouter(auth, newTestVideoModule(t), limiter, newChecker()), tokens
}
...
func TestAScrapeIsNotCountedAgainstARateLimitBudget(t *testing.T) {
	...
	if recorder := serveRequest(t, router, http.MethodGet, "/api/status", "Bearer "+token); recorder.Code != http.StatusTooManyRequests {
...
//
// The two parameterized routes are driven here on purpose: they are the only
// place in this service where an identifier is part of the request line, so
// they are where the route label would carry one.
func TestNoLabelValueLooksLikeAnIdentifier(t *testing.T) {
	...
	const jobID = "9b2f1c44-7d3a-4f2e-8a1b-5c6d7e8f9a0b"

	serveRequest(t, router, http.MethodGet, "/api/status", "Bearer "+token)
	serveRequest(t, router, http.MethodGet, "/api/video-jobs/"+jobID, "Bearer "+token)
	serveRequest(t, router, http.MethodGet, "/download/frames_"+jobID+".zip", "Bearer "+token)
...
// TestAMatchedRouteIsLabelledByItsTemplate holds the property the route label
// exists for. This service is the only one with parameterized routes, so it
// is the only one where labelling by the request path rather than by the
// matched template would be invisible in a count and catastrophic in a
// series: one job identifier in a label is one series per job, forever.
func TestAMatchedRouteIsLabelledByItsTemplate(t *testing.T) {
	router, tokens := newMetricsTestRouter(t, alwaysAllowRateLimiter{})
	_, token := issueTestToken(t, tokens, "3fa85f64-5717-4562-b3fc-2c963f66afa6")
	const jobID = "9b2f1c44-7d3a-4f2e-8a1b-5c6d7e8f9a0b"
	...
	samples := scrapeSamples(t, router)
	for _, route := range []string{"/api/video-jobs/:id", "/download/:filename"} {
		if !hasRouteLabel(samples, "fiapx_http_requests_total", route) {
			t.Errorf("a request that matched %q is not counted under that template", route)
		}
	}
	for _, point := range samples {
		for name, value := range point.labels {
			if strings.Contains(value, jobID) {
				t.Errorf("%s{%s=%q} carries a path parameter's value", point.name, name, value)
			}
		}
	}
}

// durationBuckets reads the duration histogram's cumulative bucket counts for
// one route, keyed by the boundary as the exposition writes it, along with
// the total number of observations.
func durationBuckets(t *testing.T, router http.Handler, route string) (map[float64]uint64, uint64) {
	t.Helper()
	...
	for _, metric := range families["fiapx_http_request_duration_seconds"].GetMetric() {
		matched := false
		for _, pair := range metric.GetLabel() {
			if pair.GetName() == "route" && pair.GetValue() == route {
				matched = true
			}
		}
		if !matched {
...
// TestAnUploadsDurationLandsInAFiniteBucket drives a real upload through the
// real router and asserts its observation is counted below the histogram's
// highest finite boundary.
//
// This is the assertion the bucket set exists for and the one no amount of
// reading the numbers replaces. POST /upload streams the request body into
// the bucket before it answers, so its handler duration includes the client's
// own transfer; the client library's default ceiling of 10s would put every
// real upload in the overflow bucket, where a histogram reports nothing at
// all while looking perfectly healthy.
func TestAnUploadsDurationLandsInAFiniteBucket(t *testing.T) {
	router, tokens := newMetricsTestRouter(t, alwaysAllowRateLimiter{})
	server := httptest.NewServer(router)
	defer server.Close()
	_, token := issueTestToken(t, tokens, "3fa85f64-5717-4562-b3fc-2c963f66afa6")

	uploadVideoAccepted(t, server.URL, token, generateTestVideo(t, 1), "metrics.mp4")

	buckets, observations := durationBuckets(t, router, "/upload")
	if observations == 0 {
		t.Fatal("the upload was not observed by the duration histogram at all")
	}
	...
	if highest < 60 {
		t.Fatalf("the highest finite bucket boundary is %gs; an upload's duration tracks the client's transfer and needs a ceiling of at least 60s", highest)
	}
	if buckets[highest] != observations {
		t.Fatalf("%d of %d upload observations landed above %gs, in the overflow bucket", observations-buckets[highest], observations, highest)
	}
}
...
// newReservationTestServer wires the real router over the in-memory
// idempotency store, and hands back both the server the upload helpers need
// and the engine the scrape is read from.
func newReservationTestServer(t *testing.T, store *fakeIdempotencyStore) (*httptest.Server, *gin.Engine, testTokens) {
	t.Helper()

	auth, tokens := newTestAuthenticatorWithTokens(t)
	module, _, _ := newIdempotencyTestVideoModule()
	module.idempotency = store
	router := setupRouter(auth, module, alwaysAllowRateLimiter{}, newChecker())
	server := httptest.NewServer(router)
	t.Cleanup(server.Close)
	return server, router, tokens
}

// TestEveryReservationOutcomeIsCounted covers all four branches the handler
// distinguishes. The conflict branch is the one an implementation is most
// likely to fold into duplicate, and the two are different events with
// different actions: a duplicate is the feature working, and a conflict is a
// request refused because another request holds a reservation it has not
// finished.
func TestEveryReservationOutcomeIsCounted(t *testing.T) {
	const family = "fiapx_upload_idempotency_reservations_total"

	for _, tc := range []struct {
		outcome string
		drive   func(t *testing.T, server *httptest.Server, store *fakeIdempotencyStore, token string, userID string)
	}{
		{
			outcome: "reserved",
			drive: func(t *testing.T, server *httptest.Server, _ *fakeIdempotencyStore, token, _ string) {
				uploadVideoAccepted(t, server.URL, token, writeTestUploadContent(t, []byte("first upload")), "reserved.mp4")
			},
		},
		{
			outcome: "duplicate",
			drive: func(t *testing.T, server *httptest.Server, _ *fakeIdempotencyStore, token, _ string) {
				path := writeTestUploadContent(t, []byte("identical bytes, twice"))
				uploadVideoAccepted(t, server.URL, token, path, "duplicate.mp4")
				uploadVideoAccepted(t, server.URL, token, path, "duplicate.mp4")
			},
		},
		{
			outcome: "conflict",
			drive: func(t *testing.T, server *httptest.Server, store *fakeIdempotencyStore, token, userID string) {
				content := []byte("content whose reservation is never finalized")
				key, err := videodomain.NewIdempotencyKey(userID, sha256Hex(content))
				if err != nil {
					t.Fatalf("unexpected error: %v", err)
				}
				if _, reserved, err := store.Reserve(context.Background(), key); err != nil || !reserved {
					t.Fatalf("failed to pre-seed the reservation: reserved=%v err=%v", reserved, err)
				}
				resp, _ := uploadVideo(t, server.URL, token, writeTestUploadContent(t, content), "conflict.mp4")
		...
		{
			outcome: "failed_open",
			drive: func(t *testing.T, server *httptest.Server, store *fakeIdempotencyStore, token, _ string) {
				store.reserveErr = errRateLimitStoreDown
				uploadVideoAccepted(t, server.URL, token, writeTestUploadContent(t, []byte("reserved by nobody")), "failed-open.mp4")
			},
		},
	} {
		t.Run(tc.outcome, func(t *testing.T) {
			store := newFakeIdempotencyStore()
			server, router, tokens := newReservationTestServer(t, store)
			userID, token := issueTestToken(t, tokens, "3fa85f64-5717-4562-b3fc-2c963f66afa6")

			before := counterValue(t, router, family, "outcome", tc.outcome)
			tc.drive(t, server, store, token, userID.String())
			after := counterValue(t, router, family, "outcome", tc.outcome)

			if after-before != 1 {
				t.Fatalf("the %s path was counted %g time(s), want 1", tc.outcome, after-before)
			}
		})
	}
}
