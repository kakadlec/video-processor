...
func TestTheReadinessCheckerConsultsExactlyItsMatrixRow(t *testing.T) {
	checker := newReadinessChecker(nil, nil)
	...
	want := []string{dependencyDatabase, dependencyObjectStorage}
...
// newObjectStorageReadinessCheck builds the object-storage check against the
// real object store, the way setupVideo builds it: a bounded, read-only
// bucket-presence call over the client and bucket startup resolved. The
// bucket is created here and the caller is handed its name so it can remove
// it mid-test.
func newObjectStorageReadinessCheck(t *testing.T) (func(context.Context) error, string) {
	t.Helper()
	...
	return func(ctx context.Context) error {
		return videostorage.CheckBucket(ctx, client, bucket)
	}, bucket
}

func TestObjectStorageReadinessPassesWhileTheBucketIsPresent(t *testing.T) {
	check, _ := newObjectStorageReadinessCheck(t)
	checker := newChecker(readinessCheck{name: dependencyObjectStorage, probe: check})
...
// TestObjectStorageReadinessFailsWhenTheConfiguredBucketIsGone exercises the
// case a reachability ping cannot see and the case this change originally got
// wrong: the object store answers perfectly well, and the bucket every route
// names its objects inside has been removed.
func TestObjectStorageReadinessFailsWhenTheConfiguredBucketIsGone(t *testing.T) {
	check, bucket := newObjectStorageReadinessCheck(t)
	checker := newChecker(readinessCheck{name: dependencyObjectStorage, probe: check})
	...
	if err := client.RemoveBucket(context.Background(), bucket); err != nil {
		t.Fatalf("remove bucket %q: %v", bucket, err)
	}

	failing := checker.failingDependencies(context.Background())
	if len(failing) != 1 || failing[0] != dependencyObjectStorage {
		t.Fatalf("expected [%q], got %v", dependencyObjectStorage, failing)
	}
