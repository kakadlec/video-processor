...
// permitted exception to "the worker constructs no HTTP server".
...
// in-flight scrape to finish. Short, deliberately: unlike the job in flight
// that this process's own shutdown otherwise waits out (up to drainTimeout),
// a stalled scrape has nothing this process is responsible for finishing.
...
// is to process video jobs, not to serve them.
//
