...
- **WHEN** an authenticated user makes a request to a rate-limited route
- **THEN** the middleware's `Allow` call is bounded by that timeout, after which the request proceeds to its handler (fail open) rather than hanging indefinitely — verified against a real (non-fake) Redis client, not only a fake that honors `ctx.Done()` by construction
