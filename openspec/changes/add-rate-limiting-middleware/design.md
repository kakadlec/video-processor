...
- **[Risk]** Fail-open only helps availability if the failure surfaces quickly — caught during review (Copilot, PR #150): the shared `*redis.Client` is opened with only `Addr` (no explicit dial/read timeout configured beyond `go-redis`'s own defaults), so an outage where connections hang rather than fail fast (e.g. a network partition, not a clean "connection refused") could make every authenticated request stall for that default timeout before falling through to "allow." → **Mitigation**: `rateLimitMiddleware` wraps the `ctx` it passes to `Allow` in a short, fixed `context.WithTimeout`. **Correction (Copilot, PR #151, round 2 — the first mitigation attempt didn't actually work):** wrapping the context alone has no effect on the real client. go-redis v9 defaults `ContextTimeoutEnabled` to `false`, in which case `baseClient.context()` silently substitutes `context.Background()` for the caller's context on every command — a `context.WithTimeout` around the `Allow` call was being discarded entirely, and a hung connection would actually be bounded only by the client's own multi-second default `ReadTimeout`, not the intended few-hundred-millisecond window. The real fix is `internal/platform/redis.Open` setting `ContextTimeoutEnabled: true` on the shared client (safe for existing callers like `idempotency.RedisStore`, which pass a plain `context.Background()`/request context with no tight deadline of their own — enabling this option only changes behavior for a caller that actually sets one, which today is only this rate limiter). With that in place, the middleware's `context.WithTimeout` is what bounds the call, not a change to the shared client's own default timeouts — but it only works *because* the client is configured to honor it. Verified with a real (non-fake) client: `internal/platform/ratelimit`'s `TestAllow_RespectsContextDeadline_RealClient` calls `Allow` with an already-expired context against a live local Redis and asserts the call fails near-instantly, rather than only asserting a context-aware fake respects `ctx.Done()` (which the first attempt's `TestRateLimitMiddleware_UnresponsiveLimiterFailsOpenWithinBoundedTimeout` did — that test is still valid for the middleware's own logic, but it couldn't have caught this bug since the fake `videoRateLimiter` always honors context itself, by construction).
