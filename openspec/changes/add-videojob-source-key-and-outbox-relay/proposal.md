...
- New `Relay` in `internal/video/infrastructure/messaging`, composing that repository with a `Publisher` over the `internal/platform/rabbitmq` connection. It publishes to `JobDispatchTopology()` with delivery mode 2 and **no** per-message `expiration`, on a channel in confirm mode, and stamps `published_at` only for messages the broker acknowledged.
