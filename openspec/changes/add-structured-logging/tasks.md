...
- [ ] 2.5 Verify the disclosure test actually catches the cases it exists for — **two** temporary call sites, not one: a `NotificationPreference` passed to an emitting call, and a second bound through `With`. Confirm the test names the file and line for each, then remove both. A source-level test that has never been seen failing is a test of nothing, and the `With` clause is the half that would otherwise ship untried.
