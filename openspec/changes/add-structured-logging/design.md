...
138 package-level `log.Print*` call sites outside tests, 10 `log.Fatal*` sites, six calls through the one injected `*log.Logger`, and four `fmt.Println` startup banners — 158 emitting sites, spread over five composition roots and five `internal/` packages. Counted by parsing the tree rather than grepping it: a text search reports 13 fatal sites, three of which are `log.Fatal(r.Run(...))` inside explanatory comments. All of them write prose to the standard library's package-level logger. `proposal.md` states why that is a problem; this document settles how it is replaced.
...
*Alternative considered — zerolog:* better ergonomics for chained fields and a faster encoder. Rejected: a dependency added to make 158 already-written call sites marginally nicer to read is a poor trade in a repository that pins its architecture with tests and treats every dependency as surface.
