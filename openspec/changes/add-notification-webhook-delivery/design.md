...
The guarantee about `Secret.Reveal()` is about *reading a stored secret*, and has to be written that way, because it is already false as a statement about the codebase: `internal/notification/infrastructure/postgres`'s `PreferenceRepository.Set` calls `Reveal()` on a secret its own caller handed it in the same call, in order to write it to the column. That is the write path this context has always had and it stays. What this change adds is the first caller that reveals a secret *loaded back out of storage*, and that caller SHALL be the signer alone — which is what the test asserting no path under `cmd/api` reaches `FindDeliverable` actually checks. The delivery record stores no secret. Log lines name the triple and the delivery id, never the body and never the destination's query string — and, per the spec, never a transport error's own text, since `*url.Error` renders the full URL including a credential a destination may carry in its query.

### The dial guard is worthless with a proxy in the environment

`Dialer.Control` is the right hook — it runs after resolution, with the address about to be connected — but it guards only the connection the transport actually opens. Set `HTTPS_PROXY` and that connection is to the proxy: a public, duly-approved address, after which the proxy resolves the user's hostname and connects to whatever it finds, inside the network or not. The entire prefix table is then bypassed by an environment variable that may have been set for an unrelated reason, and nothing in the code looks wrong.

So the transport's `Proxy` is nil, explicitly, and `http.ProxyFromEnvironment` — which is what `http.DefaultTransport` uses, and therefore what a hand-rolled transport is most likely to be copied from — is not used. Stated as a requirement rather than left as a code detail, because the failure is silent and the fix is one field.
