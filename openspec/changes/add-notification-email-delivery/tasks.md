...
- [ ] 5.3 Confirm no budget term is added and that `DeliveryConfig.MaxClaimHold()` and `Validate()` are unchanged; add a test asserting the SMTP attempt is bounded by the same injected timeout the webhook attempt uses.
- [ ] 5.3a Size the shutdown drain from the channel count: a message can resolve to one preference per channel and they are handled sequentially, so `MaxClaimHold() + drainGrace` no longer covers a message that delivers on both. Derive it from the closed channel set so a later channel moves it automatically, and update `stop_grace_period` for `notifier` in `docker-compose.yml` to stay above it.
