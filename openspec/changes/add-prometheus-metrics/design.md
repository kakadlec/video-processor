...
- **gin can enumerate its own routes — and that enumeration stops at the composition root.** `(*gin.Engine).Routes()` returns `[]RouteInfo{Method, Path, …}` (`gin@v1.12.0/gin.go:68-76`, `:390`), so the route-label domain is readable from the router at startup rather than maintained by hand. What the shared package receives is **not** that type: no non-test file under `internal/` imports the framework today, and `internal/platform/logging/dependency_test.go:80-103` states that as a rule about the platform namespace rather than as an accident.
...
- **`video_jobs` is indexed for `WHERE status = 'processing'` and for nothing else status-shaped, and that one index is keyed by `id`** (`schema.sql:64-68`, plus a `(user_id, created_at DESC, id ASC)` listing index). A `GROUP BY status` gauge would sequentially scan the whole job history on every scrape. **Neither half of the gauge is free, contrary to an earlier draft of this document.** `video_jobs_processing_id_idx` is keyed by `id` because the recovery sweep resumes from a keyset cursor on `id`, so it answers *which rows are processing* and cannot answer *which of them is oldest* without reading all of them; `MIN(created_at)` over it is a scan of the matching set, not a one-row read. The `queued` predicate has no index at all. Two states, two oldest-age lookups, one existing key order that serves neither.
...
| `fiapx_upload_idempotency_reservations_total` | `outcome` ∈ `reserved`, `duplicate`, `conflict`, `failed_open` | `duplicate` is what the feature is for; `failed_open` means deduplication is off and repeat uploads are running `ffmpeg` again; `conflict` is the `409` a request receives when a reservation it did not win never resolves inside its bounded wait, which is ordinarily rare and, when it rises, means holders are dying between reserving and finalizing. Four outcomes because the handler distinguishes four (`cmd/video-api/video.go:670-707`) — three would leave the family's sum short of the uploads that reached the reservation, with nothing saying where the difference went. |
...
- *Anything recorded at a site that **only** `cmd/worker` or `cmd/notifier` runs.* Not because instrumenting them is hard, but because a metric no process exposes has no value now, and proposing one would put decision 1's answer in contradiction with this section. The qualifier is load-bearing and is the one place this section and decision 7 can be read as disagreeing: `internal/video/infrastructure/cache` **is** instrumented and the worker **does** increment its counter, because that site is one an exposing process runs too. What the worker holds is an unscraped register of families it shares with `cmd/video-api` — not a family added for its benefit. The follow-up change adds a handler; the instrumentation only the worker would run is that change's work, not this one's.
- *A `notification_deliveries` aggregate.* No index serves it (see Context). The refusal is **not** an index budget — this change adds two, both to `video_jobs`, both for the one blind spot nothing else in the system can see. It is that the notifier's stall is a different problem in a different context: those rows live in the Notification database, which `cmd/video-api` neither holds a pool for nor may reach, so the gauge would mean giving the exposing service a second context's connection purely to observe a third process. Instrumenting the notifier is the answer to that, and it is the follow-up row rather than this change.
...
1. **Every label value is a string literal, or a call to `metrics.Route(…)` or `metrics.Method(…)`.** This covers `WithLabelValues` arguments, the values *and the keys* of a `prometheus.Labels{…}` composite literal, and the trailing label values of `MustNewConstMetric`. The two permitted constructors are the exact analogue of logging's `slog.String/Int/Int64/Bool/Duration/Time` allow-list: a closed set of functions, each of which is **total** — it returns a member of a bounded set for every input, including inputs it does not recognize.

   **The rule is a permission with a failing default, not a list of prohibited forms, and that distinction is the whole of its strength.** Enumerating the positions above and passing everything else is bypassed in one line: `WithLabelValues(values...)` spreads a slice and `With(prometheus.Labels(m))` converts a map, and neither call site contains a literal or a permitted constructor anywhere for an enumerating walk to catch — so a rule advertised as making cardinality structurally impossible would in fact permit both. An argument the walk cannot resolve to a literal or to one of the two constructors therefore **fails**, including an argument whose syntactic form the walk does not recognize at all. The practical consequences: a spread argument fails; a `prometheus.Labels` argument that is not a composite literal fails; a computed key inside such a literal fails, because a key is a label *name* chosen at runtime and rule 2 covers only the names written at declaration; and a form a later version of the client library introduces — `CurryWith`, a helper returning `Labels`, whatever else — fails on arrival rather than passing unnoticed until someone thinks to add it. The cost is that a legitimate new form has to be admitted deliberately, which is the correct direction for this rule to be wrong in.
...
**Two labels are resolved rather than written — the matched route and the request method — and both are bounded by this one table.** The requirement names both: describing the route alone would leave `metrics.Method` permitted by rule 1 and outside the contract that bounds it, while the ceiling below multiplies by methods either way.

`metrics.BindRoutes(…)` runs once in each `setupRouter`, after every route is registered, and takes **this repository's own route representation — a method and a path per entry — rather than `gin.RoutesInfo`.** Taking the framework's type would put the first `internal/` import of gin into the shared package, for a value that is two strings, and against a rule the platform namespace states about itself (`internal/platform/logging/dependency_test.go:80-103`); since the new package carries its own copy of that dependency test, the design would have failed its own test. Each root converts `engine.Routes()` at the call site, which is in any case the only place that knows which router it is talking about.

`metrics.Route(path)` returns `path` when the route table contains it and the fixed literal `<unmatched>` otherwise; `metrics.Method(m)` returns `m` when it is one of the table's methods and the fixed literal `UNRECOGNIZED` otherwise — reusing the access record's own spelling so the two fields read the same in a log line and in a series.
...
The four pipeline gauges are a `prometheus.Collector` whose `Collect` runs the aggregate statements, bounded by a context timeout of its own, against the video pool `cmd/video-api` already holds.

**The aggregates are methods on the concrete `postgres.Repository`, not on `domain.VideoJobRepository`, and the collector lives in that same package.** Widening the domain port would oblige the cache decorator and every application-layer test double to carry two methods that exist for one collector in one process — the cost a port exists to avoid — and it would make it *possible* to wire the collector through the decorator, which is the one thing that must not happen here, since a cached count is a count from a different moment. Keeping them concrete does both jobs at once: `*CachedVideoJobRepository` is not `*Repository`, so the collector cannot be built on it. That is a compile error rather than a review comment, and it is the posture `GET /download/:filename`'s entitlement lookup already takes against the same decorator for the same kind of reason. If a seam is ever wanted, this repository's own idiom is a consumer-declared unexported interface in the reading package — `messaging.outboxClaimer` over `postgres.OutboxRepository` — not a wider domain port.
...
**And because absence is how failure is reported, emptiness has to be reported as a zero.** A bare `GROUP BY status` returns no row for a state with no jobs, so a drained queue would vanish from the exposition by the very mechanism a dead database uses, and the distinction the paragraph above is built on would be unmakeable at the moment it matters — quite apart from leaving a series that disappears and returns, which no range query reads across. Every count therefore carries a sample for every member of its closed label set, zero where nothing matched. The **age** is the one value legitimately missing from a successful scrape, because an empty state has no oldest row to describe: a healthy idle system reads as every count at zero and no age at all, and that combination is a signature rather than a gap.

That settles the statement's shape too, and not on aesthetic grounds: the per-state predicate is written as a **literal**, one statement or `UNION ALL` branch per state, because a partial index whose predicate is a literal is matched only when the planner can prove the query's predicate implies it — which it cannot do for a parameter whose value it does not yet have, so a tidy `WHERE status = $1` would quietly forfeit the index this decision and decision 9 are both spending on. The closed set the zero-filling needs and the literal predicate the index match needs are the same closed set.

**The count saturates at 10,000; the age does not.** A count over a partial index costs one index entry per matching row, which is nothing when healthy and is proportional to the backlog when broken — and the backlog is exactly the case that repeats every scrape interval forever. The count is therefore taken over a bounded subquery and saturates, while the oldest-age gauge is a one-row index lookup that never does. **The bound is written as a number rather than left to implementation**, because the statement's shape, the gauge's meaning and the test that asserts saturation all read the same value and three artifacts choosing it independently is how they come to disagree; `videojob-persistence` is where it is normative and every other artifact refers to it. The value comes from the healthy range: three workers at prefetch 1 make a healthy `processing` count three and a healthy `queued` count single digits, so 10,000 sits about three orders of magnitude clear of normal, while a bounded index-only read of at most 10,000 entries stays under a millisecond. A saturated count still says *at least this many*, and the age says how bad it is, which is the number an operator acts on: a `queued` count of 40 means nothing on its own, and an oldest-`queued` age of eleven minutes means the workers are gone.

### 9. Two new partial indexes, and two states rather than five

`fiapx_video_jobs_in_state` carries `queued` and `processing` and no other state. Each exclusion has a reason:

- `queued` is the one that matters most and the one nothing serves at all. It gets a partial index on `video_jobs (created_at) WHERE status = 'queued'`, shaped like the sweeper's beside it. It is what distinguishes "uploads are being accepted and nothing is consuming them" from "the system is idle" — the exact condition `docs/operations.md:751` records as currently undetectable.
- `processing` gets one too — `(created_at) WHERE status = 'processing'` — and calling it free was this document's own mistake. `video_jobs_processing_id_idx` exists, but it is keyed by `id` to match the sweep's keyset cursor: it finds the processing rows and cannot order them by `created_at`, so an oldest-age lookup over it reads every matching row instead of one. **The honest cost of shipping it that way would have been small**, and the reason for the index is not the scan: a `processing` set is bounded by claimed jobs, three workers at prefetch 1, and a sweeper that requeues three times and then commits `failed`, so it is hundreds of rows in the worst realistic case rather than the growing history the `queued` predicate risks. The reason is that **the guarantee has to be uniform to be checkable** — task 5.3 EXPLAINs both statements and asserts each oldest-age lookup reads a single ordered row, and a per-state exception makes that a rule with an asterisk that the next reader has to rediscover from the key order of an index written for something else.
- **The schema change is therefore two partial indexes, not one.** A job enters and leaves each on the same edges by which it enters and leaves the sweeper's index, so the write cost is a shape the schema already accepts; what is new is that a `processing` row carries two partial entries rather than one, which is the price of two questions — a keyset cursor and an oldest-row lookup — that no single key order answers.
...
`collectors.NewGoCollector()` and `collectors.NewProcessCollector(collectors.ProcessCollectorOpts{})` go into the explicit registry — the second with its options value written out, because at the pinned `client_golang v1.24.1` the signature is `NewProcessCollector(opts ProcessCollectorOpts)` and the zero value is what selects the defaults, while `NewGoCollector` is variadic and correctly takes none. Spelled out at this level because the call written the other way does not compile, and a design a reader has to correct before typing it is a design nobody checked. They are the cheapest available answer to a class of failure nothing else in this system can see: a goroutine leak (the relays, the sweeper, the consumers and the heartbeat all run goroutines), heap growth, GC pressure, and file-descriptor exhaustion. Their cardinality is fixed, known and small.
...
- **A database query on every scrape, from the service that also serves uploads** → One read per in-flight state and per relayed event type, every predicate index-served and written with a literal so the partial index is matched, every read context-bounded, each count saturating at 10,000. No caching, on `storage.Ping`'s stated grounds. The alternative — a background refresher — reintroduces exactly the staleness that decision removed.
...
1. `internal/platform/metrics`: the registry, the handler, the two label constructors, the framework-neutral route-table binding, the three-rule AST walk, and the package's own `dependency_test.go` — the platform placement rule is per-directory and a new package carries it or is unenforced, which the logging copy says in its own doc comment. The walk lands with the package, before there is anything for it to catch, so the first family added is judged by it.
...
4. The two new partial indexes in `internal/video/infrastructure/postgres/schema.sql`, then the aggregate methods on the concrete repository, then the collector beside them, then its registration in `cmd/video-api` over the undecorated repository.
5. The gateway's third refusal block.
6. Documentation and the roadmap row, in the finalization pass.

**Rollback**: revert. The change adds one route per HTTP service, two indexes, a set of read-only aggregate reads and a set of counters; it writes no application state, changes no wire format, no response and no shutdown sequence, and nothing in the system reads its output. The indexes are the only artefacts a revert leaves behind, and `DROP INDEX` is optional rather than required — unused, they cost one entry per `queued` job and one per `processing` job, on rows that are transient by definition.
