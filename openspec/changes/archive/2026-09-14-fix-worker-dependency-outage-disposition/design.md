...
`ClaimForProcessing` is **two** autocommit statements, and the second runs on one branch only (`Repository.ClaimForProcessing` in `internal/video/infrastructure/postgres/repository.go`):
...
A failure of either statement carries the same sentinel, and the design is argued from what holds on **both** branches: *this worker did not learn its claim's outcome*. The sentinel deliberately asserts nothing stronger, although its probe branch could. Across the two, a failure the caller sees admits exactly two outcomes for this claim and no third:

- **The claim did not commit.** The row is as the claim found it — `queued` if the `UPDATE` never took effect, whatever non-`queued` state it already held on the probe branch — and this worker has acquired no lease, read no source object, run no `ffmpeg` and written no outbox row. A redelivery is not a *retry* — it is the original delivery, arriving later, and the very same conditional claim will decide it, including dead-lettering it as a lost claim or an unknown job if that is what the row says by then.
...
- **A reviewer reading the lease-recovery clause as contradicted.** *"Recovery SHALL NOT be built on broker redelivery"* remains true and is about a different subject: a job already `processing`, whose redelivery cannot be claimed. This change redelivers a dispatch that was **never acted on** — for a row still `queued`, or, when the claim committed and lost its result, for a `processing` row with no lease whose redelivery dead-letters and whose recovery is the sweeper's ordinary path, not an exception to the clause. The delta states the distinction in the requirement itself rather than leaving it to be inferred, because the sentence is otherwise the first thing someone will quote against this design.
