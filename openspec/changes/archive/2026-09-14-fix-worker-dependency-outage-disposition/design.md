...
- **`ProcessVideoJob.Execute` propagates the claim step's error at exactly one site.** `ProcessVideoJob.Execute` in `internal/video/application/process_video_job.go`, immediately after its `uc.start.Execute` call. There is one place to mark the pre-claim region, and it is a few lines long. (Line numbers cited in this section are against the proposal's base commit and have since drifted; where a later decision names the site of a change, it does so by symbol.)
...
The claim step makes up to **three** persistence calls before this worker knows whether it holds the job. The first is not part of the claim at all: `StartProcessing` loads the job through the authoritative repository, and only then does `ClaimForProcessing` run its **two** autocommit statements, the second on one branch only (`Repository.ClaimForProcessing` in `internal/video/infrastructure/postgres/repository.go`):

- **The authoritative load**, `FindByID`, which precedes any claim (`StartProcessing.Execute` in `internal/video/application/start_processing.go`). A failure here means no claim was attempted, and the row is in whatever state it already had — missing, `pending`, `queued`, `processing` or terminal.
- **The claim**, a conditional `UPDATE ... RETURNING lease_epoch` whose result is read with `QueryRowContext(...).Scan(&epoch)` over the connection that carried it. There is no client-visible boundary between its commit and the delivery of its result, so a failure the caller sees here is genuinely ambiguous: not committed, or committed with the result lost.
- **An existence probe**, `SELECT 1 FROM video_jobs WHERE id = $1`, reached **only** when the claim's `Scan` returned `sql.ErrNoRows`. From an `UPDATE ... RETURNING` that proves zero rows were affected, so on this branch the claim **provably did not commit**; the probe exists only to tell a lost claim from a job that does not exist.

A failure of any of the three carries the same sentinel, and the design is argued from what holds on **all three**: *this worker did not learn its claim's outcome, and produced no side effect* — no lease, no source read, no `ffmpeg`, no outbox row, no terminal write. The sentinel deliberately asserts nothing about the stored row, although its probe branch could. What makes that enough is how the redelivery decides the row:

- **Whatever the row holds, the redelivery re-runs the same authoritative load and the same conditional claim.** A missing row yields `ErrVideoJobNotFound` and dead-letters; a `pending` row is refused by the aggregate and dead-letters; a `queued` row is claimed; a `processing` or terminal row yields `ErrJobClaimLost` and dead-letters, a `processing` one belonging either to a live holder or, if abandoned, to the sweeper. A redelivery is not a *retry* — it is the original delivery, arriving later, decided exactly as the original would have been.
- **Exactly one branch necessarily becomes the sweeper's input: the claim committed and its result was lost** — a connection dropped between `COMMIT` and the client reading the returned epoch. The row is `processing` and holds **no lease**, because `ProcessVideoJob` acquires the lease only at the moment `StartProcessing` reports a *won* claim and here it reported an error. That is exactly `videojob-lease-recovery`'s definition of an abandoned job: the redelivery loses the claim and dead-letters under the existing rule, two unleased observations at the same epoch requeue the row at a fresh epoch, and the ordinary path resumes.

Every state the row can hold is decided by a mechanism that already exists, and the disposition is correct under each. **That is what makes the boundary sharp — not that nothing happened, but that everything that could have happened is already owned.** An earlier draft of this document argued the near side was "bit-for-bit unchanged". It is not, the source disproves it, and the argument never needed it.

On the far side there is no disjunction at all — the claim was reported won, so the row is `processing`, a lease is held, and work has begun — and the existing rule is correct for exactly that reason: the claim predicate admits `queued` alone, so a redelivery can only lose the claim and be dead-lettered, which `videojob-lease-recovery` already states as its reason for not building recovery on broker redelivery. What the committed-and-lost branch shares with the far side is the row's status and nothing else; it is the *absent lease* that makes it the sweeper's, and that is why a failure after a won claim cannot borrow this disposition.

So the new disposition applies to one condition: **the claim step could not learn whether the claim was won**, because the persistence layer could not answer. Not "an error occurred before the claim" — decision 2 explains why that is not the same sentence, and not "the claim provably did not happen", which decision 1 has just shown the claim statement cannot provide, and which only the probe branch could.

*Corrected at finalization.* This decision was first written as though `ClaimForProcessing` were one statement, and task 1.6 asked for that to be confirmed. It never was: the existence probe was already there at the proposal's base commit. Nothing downstream changed, because the argument never depended on how many statements there are — only on what this worker did before the failure, and on each outcome the claim could have reached having an owner.

*Corrected again at finalization review (2026-09-14).* It was also written as though the claim step began at the claim. `StartProcessing`'s authoritative `FindByID` runs first and raises the same sentinel before any claim is attempted — it is the call that failed in task 6.6's live-stack run, where PostgreSQL was stopped before the worker reached the claim — so on that branch the row may hold any state at all. The argument above now covers all three origins and asserts nothing about the row; what it rests on, that this worker produced no side effect and that the redelivery's own load and claim decide the row, is unchanged.
...
The candidate evidence, to be *confirmed* against the pinned `github.com/jackc/pgx/v5 v5.10.0` and against `database/sql`'s own pooling at implementation time rather than pinned normatively in the spec, was: `*pgconn.ConnectError` (the connection attempt failed — and note it does **not** satisfy `pgconn.SafeToRetry`, which has no method on it and unwraps to a plain net error, so that helper is not the classifier), `pgconn.ErrConnClosed`, `driver.ErrBadConn`, `sql.ErrConnDone`, any `net.Error` reached by `errors.As`, `io.EOF`/`io.ErrUnexpectedEOF`, and a `*pgconn.PgError` whose `SQLState()` falls in class `08` (connection exception), `53` (insufficient resources), `57` (operator intervention) or `58` (system error).

**What shipped is narrower, and the narrowing is the outcome of the empirical pass task 1.7 required, not an omission.** Each candidate was provoked through `database/sql` against the pinned driver and admitted only if it was observed reaching a caller. The classifier, `isUnavailable` in `internal/video/infrastructure/postgres/errors.go`, admits:

- a `*pgconn.PgError` whose SQLSTATE is one of **three enumerated states** — `57P01 admin_shutdown`, `57P03 cannot_connect_now`, `53300 too_many_connections` — and never a class prefix. Matching on class is unsafe: class `57` also carries `57014 query_canceled`, an ordinary statement cancellation, and `57P04 database_dropped`, which is permanent, so both stay out by name. Classes `08` and `58` are absent because no provocation produced one: every transport failure arrived as a raw network or I/O error, or as `57P01`;
- a `*pgconn.ConnectError` — reached only when no `*pgconn.PgError` is inside it (see the rule order below);
- `io.ErrUnexpectedEOF`, the connection lost in flight; and
- any `net.Error` reached by `errors.As`.

`pgconn.ErrConnClosed`, `driver.ErrBadConn`, `sql.ErrConnDone` and bare `io.EOF` were not admitted; the source records bare `io.EOF` as never observed reaching a caller. **Widening the set needs the same empirical pass** — provoke the error against the pinned driver and read what actually arrives through `database/sql` — never a reading of the SQLSTATE registry or of the driver's source.

**The rule order is load-bearing in two places**, which a list of admitted evidence does not show:

1. **Context cancellation and deadline are refused first.** `context.DeadlineExceeded` itself satisfies `net.Error` with `Timeout()` true, so judged by the `net.Error` rule a caller's own deadline would read as an outage.
2. **A `*pgconn.PgError` is checked before a `*pgconn.ConnectError`, and its verdict is terminal.** A rejected password (`28P01`) and a nonexistent database (`3D000`) both arrive as a `*pgconn.ConnectError` wrapping a `*pgconn.PgError`. A `ConnectError`-first rule would mark a rotated credential as an outage and spin every replica forever; judged `PgError`-first, the server's own answer decides, and neither state is on the list.

Every SQLSTATE a `*pgconn.PgError` can carry other than those three is treated as the server answering and refusing.

**Note what decision 1's argument buys here, because it is what makes this classification tractable rather than a second hazard.** The classifier does **not** have to prove the statement never reached the server. Several of the entries above are ambiguous about that by construction — a connection lost in flight above all — and under a "the state is unchanged" argument each would have to be excluded, or handled specially, or defended. (`57014 query_canceled`, which an earlier draft cited here, is excluded from the shipped list for a different reason: it is an ordinary cancellation, not unavailability.) Under decision 1's argument each is simply safe, because every outcome is decided by a mechanism that already exists. Conceding that the state is not guaranteed unchanged is what lets the permission list be drawn on availability rather than on provable non-delivery.
...
So in `ProcessVideoJob.Execute`, at the single place it propagates `StartProcessing`'s error, a `domain.ErrRepositoryUnavailable` becomes `domain.ErrJobClaimOutcomeUnknown` (wrapping the original, so both remain reachable by `errors.Is`). The name is a sibling to `ErrJobClaimLost` and says the thing that matters: `ErrJobClaimLost` means **another actor owns this job**; `ErrJobClaimOutcomeUnknown` means **this worker cannot say who owns it, including whether it does itself** — and, either way, has run no extraction, acquired no lease, read no source object and written no event.
...
**The arithmetic, because a reviewer should not have to derive it.** The pause bounds *each consumer's* rate, not the message's. Across three replicas the message is attempted up to three times per pause — some 36 redeliveries a minute at most — so the *average* deployment-wide interval is roughly `pause / 3`, not one attempt every five seconds. That is the number to check against, and it is acceptable: each pass costs one failed connection attempt against a database that is down, and the first pass after the database returns succeeds, so a dispatch resumes within one pause.

**The attempts are not evenly spaced, and an earlier version of this paragraph said they were.** It argued that a consumer in its post-nack pause "holds nothing", so the other replicas would take the requeued message at once and the passes would spread out at about 1.7-second intervals. A live three-worker stack showed otherwise: all three instances requeued within about ten milliseconds of one another, once every 5.00 seconds, with the message sitting `unacked=1` in a pausing consumer's buffer. The nack restores that consumer's prefetch credit, so the broker pushes the requeued message into its buffer, where it waits out the pause. The attempts therefore arrive **as a burst once per pause**. The average and the recovery bound above are unaffected; only the spacing claim was wrong. The shipped doc comments on `DefaultRequeuePause` and `Consumer.dispatch` state the mechanism.
...
- **A reviewer reading the lease-recovery clause as contradicted.** *"Recovery SHALL NOT be built on broker redelivery"* remains true and is about a different subject: a job already `processing`, whose redelivery cannot be claimed. This change redelivers a dispatch this worker **never acted on**, whatever the row holds — the redelivery's own load and claim decide it — and when the claim committed and lost its result, the row is `processing` with no lease, its redelivery dead-letters, and its recovery is the sweeper's ordinary path, not an exception to the clause. The delta states the distinction in the requirement itself rather than leaving it to be inferred, because the sentence is otherwise the first thing someone will quote against this design.
