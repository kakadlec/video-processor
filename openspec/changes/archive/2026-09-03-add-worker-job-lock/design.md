...
- **A sweep observes a job in the window between its claim committing and its lease being set** → That single observation records only an in-memory mark; it writes nothing. The lease is acquired as the first act after the claim commits, and two cycles a full sweep interval apart cannot both fall inside that gap. This confirmation is required for requeue and terminal abandonment alike: at the bound the sweeper commits `failed` and deletes the source, so one mistaken observation there would kill a healthy extraction and destroy its input, which no fence can undo. Rejected mitigations: acquiring the lease *before* the claim (puts Redis back in the pickup path, decision 2's whole objection) and confirming only at the bound (the same statefulness for a narrower guarantee). The cost accepted is a worst-case recovery latency of two sweep intervals and a small in-memory, per-replica mark set — not shared, not persisted, and not what makes concurrent sweepers safe.
- **Redis is flushed or loses keys while extractions run** → Each heartbeat attempts to reacquire its missing lease. Until that succeeds the job looks abandoned, but the sweeper requires two successful not-held observations; any query error clears an earlier mark so the first observation after connectivity returns cannot complete a pre-outage pair. If a job is still requeued, the cost is duplicated work bounded by `maxRequeues`, with the fence preventing corruption. This is the same class of degradation the status cache and idempotency store already accept.
