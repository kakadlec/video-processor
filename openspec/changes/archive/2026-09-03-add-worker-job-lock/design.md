...
- **Acquiring or renewing a lease**: on error, log and proceed. The Redis-independent conditional PostgreSQL claim and terminal fence prevent a leaseless extraction from overwriting a successor, but it is invisible to the sweeper. Recovery can duplicate its work and fence it on resume; prolonged invisibility at the requeue bound can instead commit terminal abandonment and delete its source.
