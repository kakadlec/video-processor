...
Two honest limits of the check. It is a registry lookup and therefore time-of-check/time-of-use: two runs could both observe "absent" and both push. Serialization is what actually closes that, and it has to be **global rather than per version** — keyed per version it would only cover the same version published twice, while the thing publications share is `latest`: two *different* versions running at once could both read the registry, both conclude they are the highest, and let the last push win, leaving `latest` on the older image. Publishes are rare, so one lane costs nothing. The lookup remains the guard for the case that will really happen, which is a human typing a tag that already shipped. And the bootstrap publishes a tag whose tree predates this change, so its `Dockerfile` has no `$BUILDPLATFORM` pin and its non-native build emulates the Go toolchain; the binaries are still correct for their platform, only the build is slow. That is accepted for historical tags rather than fixed, because the alternative is publishing something other than what the tag contains.
...
- **Nothing in the workflow decides the package's visibility** → It is established registry-side when the package is first created, so it can be neither read off the workflow file nor assumed. Left unchecked, the documented pull command fails for exactly the audience it is written for, while every check stays green. Mitigated by verifying an unauthenticated pull after the first publication — from a genuinely logged-out client, since a logged-in one pulls a private package happily and proves nothing — and setting visibility only if that fails. It did not: `4.0.0` was anonymously pullable as published.
