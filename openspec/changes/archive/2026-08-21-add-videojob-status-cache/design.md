...
**Miss-repopulation must not clobber a newer write-through value — caught during review (Copilot, PR #154).** A naive unconditional `SET` on the miss-repopulation path races with a concurrent transition: a slow `FindByID` can miss, read `pending` from PostgreSQL, and be scheduled out; meanwhile `EnqueueVideoJob` runs end-to-end (commits `queued` to PostgreSQL, write-through `SET`s the cache to `queued`); the slow reader then resumes and unconditionally `SET`s its stale `pending` value over the fresh `queued` entry. A subsequent `StartProcessing` reads that stale `pending` value from cache and fails an otherwise-legal `queued → processing` transition. **Fix:** the miss-repopulation path uses `SET NX` (set only if absent), never a plain `SET`. Update's write-through path keeps an unconditional `SET`, since it always represents a value that was just confirmed as the latest by its own PostgreSQL commit. This ordering is safe: if a write-through `SET` already landed by the time a stale miss-repopulation's `SETNX` runs, the key already exists and `SETNX` is a no-op, preserving the fresher value; if the miss-repopulation's `SETNX` runs first (key still absent), any subsequent write-through's unconditional `SET` still overwrites it correctly regardless of ordering. A plain cache miss (genuinely no entry, no concurrent write in flight) is unaffected — `SETNX` still populates it exactly as a plain `SET` would.

**A malformed entry still needs to self-heal, without reopening the same race.** Switching miss-repopulation to `SETNX` has a side effect: `SETNX` no-ops whenever a key already exists, including a corrupted or hand-edited entry that will never deserialize — a naive `SETNX`-only repopulation would leave such an entry permanently stuck. Deleting it unconditionally before the `SETNX` would reopen the exact clobber race above (a concurrent write-through could land between the read and the delete). **Fix:** a compare-and-delete Lua script (`deleteMalformedIfUnchangedScript`, mirroring `internal/video/infrastructure/idempotency`'s own `finalizeScript`/`clearScript` atomicity pattern) deletes the key only if its current value still matches the malformed one just read. If a concurrent write-through already replaced it with a legitimate value, the compare fails and that value is left untouched; only a value that's still exactly the malformed one gets cleared, opening the way for this call's own `SETNX` to repopulate it.
