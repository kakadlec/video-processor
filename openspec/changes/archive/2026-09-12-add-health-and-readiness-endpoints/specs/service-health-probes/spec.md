...
**Note, 2026-09-12 — the canonical specification carries one rule more than this requirement does, and the gap was found in review of the implementation PR.** This text stops at cancelling the dependency call. The shipped handler and its tests go further: a probe whose caller has gone leaves the stored verdict unchanged and emits no transition record, while still answering `503`. Every check derives from the request, so a cancelled request fails all of them at once, and a handler that read that as a verdict would warn naming the whole dependency inventory and then announce a recovery from it on the next healthy probe — neither event having happened. The handler was corrected before merge and the stage that corrected it concluded the specification needed no change, which was wrong: this requirement as written is satisfiable by exactly that defect. The rule therefore lives in `openspec/specs/service-health-probes/spec.md` under *A Readiness Verdict Is Recorded When It Changes, Not When It Is Asked For*, with a pointer from this requirement's canonical counterpart. **The original text below is left standing rather than edited, because the delta was approved without it and this note is the amendment.**
