...
- [x] 1.5 `secret.go`: a `Secret` with a 16-byte minimum. Give it no `String()`/`GoString()`/`MarshalJSON` that could put the value into a log line or a response by accident — make the leak require deliberate effort.
  ...
  **Shipped as the opposite mechanism, for the stated goal.** Declaring no methods does not withhold the value: `fmt` reflects over a struct that implements nothing, so `%v` on a `Secret` — or on a `PreferenceIntent` holding one — would print the bytes. The type therefore declares `String`, `GoString` and `Format`, all rendering `notification.Secret{REDACTED}`, and a `MarshalJSON` that returns `ErrSecretNotSerializable` rather than a value, so a response struct carrying one fails at encode time instead of leaking. `%p` is the one verb `fmt` answers before consulting a `Formatter`, which is why the value is held behind an unexported pointer — reflection reaches an address there rather than the bytes, and the same indirection protects `PreferenceIntent`/`NotificationPreference`, where `fmt` cannot call methods at all. `Reveal()` is the single deliberately-named accessor. Covered by `TestSecret_IsNeverRenderedByFmt` and `TestSecret_RefusesToMarshalJSON`.

  A NUL byte is rejected alongside the length minimum, which the task did not anticipate: JSON encodes `\u0000` as a real NUL, the PostgreSQL text column cannot hold one, and without the check a malformed request would surface as a driver error and a `500`. The promoted spec carries the rule for that reason.
