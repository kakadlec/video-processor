...
- **BREAKING**: MinIO configuration becomes a startup precondition. `setupVideo` loads the config, opens the client, pings it, and ensures the bucket, failing fatally on any of those four steps — fail-closed, unlike Phase 4's Redis features, because a result that cannot be stored cannot be delivered. An existing deployment that sets only `IDENTITY_*`/`VIDEO_POSTGRES_DSN`/`REDIS_ADDR` stops booting until the operator supplies MinIO configuration. The required set is the four `minio-infrastructure` already defines as required — `VIDEO_MINIO_ENDPOINT`, `VIDEO_MINIO_ACCESS_KEY`, `VIDEO_MINIO_SECRET_KEY`, `VIDEO_MINIO_BUCKET` — with `VIDEO_MINIO_USE_SSL` staying optional. This change moves *when* `LoadConfigFromEnv` runs; it does not alter which variables that loader requires.
...
- `videojob-persistence`: the repository port gains `FindCompletedByUserID`, which filters on status in SQL and takes no offset or limit — `GET /api/status` has no pagination parameters and its filesystem predecessor was unbounded, so any limit here would silently hide a user's older results.
...
**Operations.** `docker-compose.yml`'s `app` service gains `VIDEO_MINIO_*` and `depends_on: minio`, both deliberately withheld by `add-minio-infrastructure` while nothing read them. `cmd/api`'s tests need a real bucket, so `app-test` and CI's `Test` step gain the variables the module reads. `Dockerfile` stops pre-creating `outputs`, and the `app` service drops its `./outputs` bind mount.

**Documentation.** `docs/operations.md` needs more than an env-table edit: it currently defines `outputs/` as the durable result store, warns that it accumulates without cleanup, and describes MinIO as future work. `README.md`'s limitations list says ZIPs live only on local disk, `docs/development.md` tells contributors to `chown` a directory that stops existing, and `docs/flows.md`'s sequence diagram serves the zip from `outputs/`. All are enumerated in the finalization tasks.
