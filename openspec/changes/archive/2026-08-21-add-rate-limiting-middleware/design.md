...
- **[Risk]** Fail-open only helps availability if the failure surfaces quickly — caught during review (Copilot, PR #150): the shared `*redis.Client` is opened with only `Addr` (no explicit dial/read timeout configured beyond `go-redis`'s own defaults), so an outage where connections hang rather than fail fast (e.g. a network partition, not a clean "connection refused") could make every authenticated request stall for that default timeout before falling through to "allow." → **Mitigation**: `rateLimitMiddleware` wraps the `ctx` it passes to `Allow` in a short, fixed `context.WithTimeout` (independent of whatever timeout the shared client itself is configured with), so a hung Redis degrades to "fail open within e.g. a few hundred milliseconds" rather than "stall for the client's full default timeout." This is a per-call bound at the call site, not a change to the shared client's own configuration.
