...
### 9. Terminal state writes use a detached context, not the request's own

`ProcessVideoJob`'s `FailJob` call on extraction failure, and `handleVideoUpload`'s terminal `CompleteJob`/`FailJob` calls after ownership recording, all use `application.NewFinalizationContext()` (`context.WithTimeout(context.Background(), 10s)`) instead of the incoming request context. Reusing the request context there was a real bug: if it's already canceled by that point — e.g. a client disconnect, which is also what `exec.CommandContext` uses to kill an in-flight `ffmpeg` process — the compensating persistence write would itself fail with `context canceled`, silently leaving the job stuck wherever it was instead of reaching its actual terminal state. A detached context still finishes the write regardless of what happened to the request that triggered it.

### 10. `FrameExtractor` errors are classified via sentinel errors, not exposed as raw text

`internal/video/infrastructure/ffmpeg` exposes `ErrFfmpegExecFailed`/`ErrNoFramesExtracted`/`ErrZipCreationFailed`. `ProcessVideoJobResult` carries the underlying `ExtractionError` alongside `FailureReason`, so `handleVideoUpload` can map each to the same distinct pt-BR message `/upload` always returned (`"Erro no ffmpeg: ..."`, `"Nenhum frame foi extraído do vídeo"`, `"Erro ao criar arquivo ZIP: ..."`) via `errors.Is`, instead of interpolating this package's own (English, per the language policy) error text directly into the response — which would both leak internal error text to the caller and silently change `/upload`'s long-standing error message contract.
...
- **`inMemoryVideoJobRepository`** (in `cmd/api/video_test.go`) needs an `Update` method to keep implementing `domain.VideoJobRepository`. Both it and `internal/video/application`'s `fakeVideoJobRepository` store and return independent clones of each job (via `domain.RestoreVideoJob`), matching how the real PostgreSQL adapter always reconstructs from scanned rows — otherwise a use case that mutated a job in memory but forgot to call `Update` could still pass its tests purely by aliasing the same pointer the fake already held.
...
- **`zip.Writer.Close()` is where a write failure actually surfaces**: the zip's central directory is only written on `Close`, so `createZipFile` calls it explicitly and propagates its error, rather than discarding it via a bare `defer` — otherwise a late disk/write failure there would go unnoticed and a truncated zip could be reported as a successfully completed job.
