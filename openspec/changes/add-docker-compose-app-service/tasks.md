...
- [ ] 1.1 Add a first-init SQL script (e.g. `docker/postgres-init/create-test-db.sql`) containing `CREATE DATABASE identity_test;`, mounted into the `postgres` service at `/docker-entrypoint-initdb.d/`.
- [ ] 1.2 Add an `app` service to `docker-compose.yml`: `build: .`, port published as `127.0.0.1:8080:8080` (loopback-only — see design.md), `IDENTITY_POSTGRES_DSN` pointed at the `postgres` service's `identity` database, `IDENTITY_POSTGRES_TEST_DSN` pointed at the same server's `identity_test` database, `IDENTITY_JWT_SIGNING_KEY` set, `depends_on: postgres: condition: service_healthy`.
- [ ] 1.3 Bind-mount `uploads/` and `outputs/` into the `app` container.
...
- [ ] 3.3 `CLAUDE.md`: replace the "Commands" section's `docker build`/`docker run` lines with the compose command; fix the adjacent `go run main.go` → `go run .` in the same block.
- [ ] 3.4 Do **not** modify `docs/operations.md` — its Docker section documents deployment, not local dev, and is out of scope (see design.md Non-Goals).
- [ ] 3.5 Grep `README.md`, `docs/development.md`, and `CLAUDE.md` for `docker run` / `docker build` to confirm no live-workflow reference remains outside this task list's own description text.
