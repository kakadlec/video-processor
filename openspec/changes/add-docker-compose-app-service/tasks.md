...
- [ ] 1.1 Add an `app` service to `docker-compose.yml`: `build: .`, port `8080:8080`, `IDENTITY_POSTGRES_DSN`/`IDENTITY_JWT_SIGNING_KEY` pointed at the `postgres` service over the compose network, `depends_on: postgres: condition: service_healthy`.
...
## 3. Documentation (separate PR from Configuration)
