...
- `docs/operations.md`'s "Current Deployment" → "Docker" section — that section documents actual container deployment (it includes a custom-port example that only makes sense off a real host), a different concern from local dev workflow. This change does not touch it; presenting the loopback-bound, repository-known-signing-key compose stack there would read as deployment guidance and encourage an insecure or non-functional deployment.
...
**`IDENTITY_POSTGRES_TEST_DSN` is also set on the `app` service, but pointed at a separate `identity_test` database on the same `postgres` server — not the runtime `identity` database `IDENTITY_POSTGRES_DSN` uses.** The PostgreSQL adapter test suite runs `TRUNCATE TABLE identity_users` before every test (`internal/identity/infrastructure/postgres/repository_test.go`). Pointing the test DSN at the same database the running `app` service uses would silently wipe real registered users the moment someone ran the test command — a data-loss bug, not just a design nicety. `postgres`'s existing image supports creating additional databases at first init via a script mounted into `/docker-entrypoint-initdb.d/`; a small committed SQL file (`CREATE DATABASE identity_test;`) creates the isolated test database once, at first `docker compose up`. With that isolation, `docker compose run --build --rm app go test ./... -v` safely exercises the full suite, including the PostgreSQL-backed identity adapter tests, without touching the runtime database — replacing both the old no-Postgres Docker test fallback and the separately-documented "start postgres, export the test DSN, run go test locally" flow with one command.
...
- **[Risk]** Removing the documented plain `docker build`/`docker run` path is a regression for anyone relying on it (e.g., a hackathon evaluator following the current README verbatim, or a deployment script). → **Mitigation**: `docker build -t video-processor .` and `docker run` still work mechanically — the `Dockerfile` isn't deleted, only the *documented, supported* entry point changes. This is accepted as intentional per explicit direction: one documented path, not "does the old one still technically function." This does not apply to `docs/operations.md`'s deployment-focused Docker section, which is out of scope for this change (see Non-Goals) and stays as-is.
- **[Risk]** A test database created only via a first-init script silently fails to appear if a developer already has an existing `postgres_data` volume from before this change (Postgres only runs `/docker-entrypoint-initdb.d/` scripts on a fresh, empty data directory). → **Mitigation**: document `docker compose down -v` (already an existing documented command) as the fix if `identity_test` doesn't exist; this is a one-time transition cost for existing local setups, not an ongoing issue.
...
- `CLAUDE.md`'s "Commands" section's `docker build`/`docker run` lines are replaced by the compose command (also fixing the adjacent, already-stale `go run main.go` → `go run .`, since it's the same block).
- `docs/operations.md`'s deployment-focused Docker section is explicitly **not** touched — out of scope (see Non-Goals).
