...
- `Repository.Update` — the write path behind both `CompleteJob` and `FailJob`, and its only two callers — writes a `video_job_outbox` row in the same transaction as the fenced `UPDATE`, and only when that `UPDATE` affects a row. A write refused by the fence (`ErrJobFenced`) and a write that finds its own outcome already recorded (`applied == false`) each emit nothing, so the lease-recovery sweeper and a superseded worker that both finish one job leave exactly one record of how it ended. Delivery of that record remains at-least-once, as the relay contract already says; deduplicating repeated deliveries stays the consumer's obligation and is written into the spec as such.
...
- `rabbitmq-infrastructure`: `Topology Declaration Is Idempotent And Takes A Descriptor` carries a set of work-queue routing keys instead of exactly one, so a single queue can be the destination of both terminal event types. A one-element set declares identically to before; an empty set is refused, because a direct exchange with no binding returns every mandatory publish unroutable while the declaration still reports success.

## Impact

- **Code**: `internal/video/infrastructure/postgres` (`repository.go`, `outbox.go`, `schema.sql`), `internal/video/infrastructure/messaging` (`topology.go`, `publisher.go`, `relay.go`), `internal/platform/rabbitmq/topology.go` (the descriptor's routing-key field), `internal/video/domain/repository.go` (port documentation), `cmd/worker/main.go`. `internal/video/application` is untouched — `CompleteJob`/`FailJob` keep calling `Update` and keep their `applied` semantics.
