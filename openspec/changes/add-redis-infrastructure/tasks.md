...
- [ ] 2.2 `client.go`: `Open(cfg Config) (*redis.Client, error)` — constructs a `*redis.Client` from `cfg.Addr` via `redis.NewClient(&redis.Options{Addr: cfg.Addr})`, no connectivity check.
...
- [ ] 3.2 `client_test.go`: `TestOpen_SucceedsWithoutConnecting` (unreachable address, `Open` still returns a non-nil client and no error).
...
- [ ] 4.2 `docker-compose.yml`: add `REDIS_TEST_ADDR` to the `app-test` service's environment, pointing at the new `redis` service (`redis:6379`).
...
- [ ] 6.2 Update `docs/architecture.md`: "Infrastructure Components" table (Redis row: still Phase 4, but now "connection adapter implemented" rather than fully "Planned"), "Target Package Topology" tree (add `internal/platform/redis/`), and the introductory Phase-status callout.
- [ ] 6.3 Update `docs/roadmap.md`: flip this row's status to archived in the Change Backlog under a new "Phase 4" section (create that section — this is the first Phase 4 row).
