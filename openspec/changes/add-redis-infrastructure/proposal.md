...
Distributed locking for worker job pickup — the fourth Redis responsibility the canonical spec's wording still implies — is deferred to Phase 6 (`docs/roadmap.md`, updated separately): there is no `cmd/worker` yet for workers to contend over, so a lock has no consumer until then.

## What Changes

- New `internal/platform/redis/` package: `Config`/`LoadConfigFromEnv` (reads `REDIS_ADDR`), `Open` (returns a connected client), `Close`, and a `Ping`-based health check. This is bare connection plumbing only — no idempotency store, rate limiter, or cache decorator. Those are owned by whichever future change actually implements each Phase 4 feature (idempotency and status-cache logic under `internal/video/infrastructure/`, since both wrap video-context use cases; rate-limiting middleware directly in `cmd/api`, since it isn't a bounded-context concern).
...
- New runtime dependency: a Redis instance (local dev via `docker-compose.yml`, CI via a service container) and a new third-party library (`github.com/redis/go-redis/v9`) in `go.mod`/`go.sum`.
