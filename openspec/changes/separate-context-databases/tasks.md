...
- [ ] 3.1 From a **clean** PostgreSQL volume, `docker compose up -d --build` starts every service, and each of the three `Migrate` calls creates its tables in its own database. Confirm with `\dt` in each of the three databases: seven tables total, none appearing twice.
