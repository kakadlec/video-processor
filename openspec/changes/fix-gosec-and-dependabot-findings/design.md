...
**Suppress G304 on the zip-writing paths with justification, not code changes.** `createZipFile`'s `zipPath` and `addFileToZip`'s `filename` originate from `filepath.Glob(tempDir/*.png)` and a timestamp the server itself generated — there is no user input in these two call sites. Rewriting them to avoid `os.Open`/`os.Create` would add complexity for no security benefit, so each gets an inline `#nosec G304` with a one-line justification instead.

**Suppress G204 on the `ffmpeg` invocation with justification.** Shelling out to `ffmpeg` with a path argument is the entire purpose of this service; there's no way to extract frames without it. Once `videoPath` is sanitized (previous decision) and `framePattern` is fully server-derived, the remaining "subprocess launched with variable" warning is an accepted, justified risk — documented inline rather than restructured away.
...
- [Risk] Suppressing G204/G304 with `#nosec` could mask a *future* real finding at the same line if the code changes underneath it. → Mitigation: each suppression comment includes rule ID and justification per `openspec/specs/development-workflow/spec.md`'s existing requirement, so a reviewer re-reads the reasoning whenever that line is touched.
