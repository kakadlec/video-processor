...
- [ ] 2.5a Map a violation of `notification_preferences_secret_not_empty` to a typed error the application layer renders as a client error, matching the constraint **by name** rather than by SQLSTATE alone. Every other constraint failure stays an unexpected error. Verify the driver actually exposes the constraint name on the returned error before relying on it; if it does not, fall back to the transactional `xmax = 0` variant `design.md` records as the alternative and say so there.
...
- [ ] 2.6a Adapter test against a real database: a create with no secret is refused by the constraint and stores nothing; an update omitting the secret preserves it; two concurrent first-writes of one triple, one of them omitting the secret, leave exactly one row carrying a non-empty secret. These cannot be covered by a fake repository — the constraint is the enforcement.
...
- [ ] 3.1 `SetPreference`: parse every field through the domain, surface the repository's typed "secret required" error as a validation failure rather than an unexpected one — the use case does not pre-read to decide create-vs-update, per `design.md` — and stamp `UpdatedAt` (and `CreatedAt` on insert) from an injected `Clock`, call the repository, return the stored view. Reuse the `Clock` port shape both existing contexts already have rather than calling `time.Now` in the use case.
