...
The same read/write split extends to the three use cases that decide ownership — `StartProcessing`, `CompleteJob`, `FailJob` — which take an undecorated reader and a cached writer. `EnqueueVideoJob` keeps its cached read: a submitter transitioning a job it created moments earlier is not an ownership decision, and no requeue can reach a `pending` row. This is what makes the cache's *write*-throughs safe to leave unordered with respect to one another, which they are: the requeue commits its outbox row before its cache write, so a worker can claim and cache `processing` while that call is still in flight, and either write can land last. With every ownership read on the authoritative row, the residue of that reordering is the status word `GET /api/video-jobs/:id` reports for at most the entry's TTL, and `app.js` polls identically on `queued` and `processing`.

Ordering the write-throughs instead — a compare-and-set on `(epoch, status rank)` — was designed and rejected. It works, but it redesigns the write semantics of a shipped capability in a proposal about something else, and it buys nothing the authoritative reads do not already hold. The hazard is worth naming because it is not the obvious one: a *claim's* write-through delayed past a requeue's leaves `processing` cached against a `queued` row, `job.StartProcessing()` refuses `processing → processing`, and the worker dead-letters the resulting `ErrInvalidStatusTransition` — while the sweeper, which scans `processing`, never sees that row again. That is a recovery lost permanently, and the reader split is what closes it.
...
- **A sweep observes a job in the window between its claim committing and its lease being set** → The row is requeued under a live worker: one duplicated extraction, no corruption — the old holder's terminal write is fenced, the shared result key is safe on decision 9's two conditions, and the source object is not deleted by a fenced worker. The stalled claimant's own late lease acquisition cannot clobber its successor's, because acquire refuses to replace a newer epoch (decision 5). The window is milliseconds wide and the lease is acquired as the first act after the claim commits. **That reasoning holds for the requeue and fails for the abandonment**, which is why the sweeper now requires two consecutive unleased observations at the same epoch before acting. At the requeue bound the sweeper does not requeue — it commits `failed` and deletes the source object. A sweep landing in the claim-to-acquire window there kills a healthy extraction and destroys its input, and no fence undoes either. Two cycles a sweep interval apart cannot both fall inside one process's gap between a PostgreSQL commit and a Redis write. Rejected mitigations: acquiring the lease *before* the claim (puts Redis back in the pickup path, decision 2's whole objection) and confirming only at the bound (the same statefulness for a narrower guarantee, and it leaves live extractions being needlessly re-dispatched on every other job). The cost accepted is a worst-case recovery latency of two sweep intervals and a small in-memory, per-replica mark set — not shared, not persisted, and not what makes concurrent sweepers safe.
- **Redis is flushed or loses keys while extractions run** → Every in-flight job looks abandoned and is requeued once. Duplicated work, bounded by `maxRequeues`, no corruption. This is the same class of degradation the status cache and idempotency store already accept.
- **The rolling deploy that introduces this** → A job in flight is held by a previous-build worker that sets no lease, honours no fence, and writes through the unconditional `Update` it was compiled with. If the first sweep requeues that job and a new worker takes it over, the old worker's write is not refused by anything, so it can land on top of the new holder's terminal state — the recorded outcome would then be the loser's. The aggregate check does not prevent it: the old worker loaded `processing` before the new holder committed. **Draining old workers before starting new ones is therefore a precondition of this deploy, not a mitigation**, and it is the one operational instruction this change adds. `run` already waits up to `drainTimeout` on SIGTERM, so the cost is rollout ordering, not code. Once drained, the exposure is one duplicated extraction for jobs running at the cutover, resolved by the fence.
