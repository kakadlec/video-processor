...
- [ ] 1.1 Confirm the implementation scope and HTTP contracts against the canonical DDD architecture spec.
- [ ] 1.2 Define the `User` aggregate, normalized email rules, password policy, and opaque `UserID` value object.
- [ ] 1.3 Define repository, password-hasher, and token ports without infrastructure imports in domain/application packages.
- [ ] 1.4 Define the PostgreSQL schema/migration and configuration contract, including required JWT signing configuration.

## Identity implementation

- [ ] 2.1 Implement `UserID` and `User` domain behavior with unit tests for validation and normalization.
- [ ] 2.2 Implement `RegisterUser` with duplicate detection and password hashing.
- [ ] 2.3 Implement `AuthenticateUser` with generic credential failures and token issuance.
- [ ] 2.4 Implement PostgreSQL persistence using parameterized queries and adapter tests.
- [ ] 2.5 Implement password-hash and JWT adapters with explicit algorithm and expiry validation.

## HTTP integration and security

- [ ] 3.1 Add registration and login endpoints with documented status codes and non-sensitive responses.
