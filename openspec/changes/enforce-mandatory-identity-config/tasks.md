...
- [ ] 2.1 In `setupRouterWithIdentity` (`identity.go`), remove the `identity != nil` conditionals around `requireBearerAuth()` and `requireArtifactOwnership()` so those middlewares are always attached.
- [ ] 2.2 Delete `setupRouter()` (the no-identity wrapper); confirm via `grep -rn "setupRouter()"` that no remaining caller exists outside the test files being updated in section 3.
...
- [ ] 3.3 In `identity_test.go`, replace `TestSetupIdentity_NeitherConfigured_ReturnsNilModuleNoError` with a test asserting `setupIdentity` returns a non-nil error when both `IDENTITY_POSTGRES_DSN` and `IDENTITY_JWT_SIGNING_KEY` are unset.
- [ ] 3.4 In `identity_test.go`, delete `TestSetupRouter_IdentityRoutesNotRegisteredWithoutModule` (asserts behavior that no longer exists once `setupRouter()` is removed).
