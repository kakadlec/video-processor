...
- [ ] 4.3 Assert a panic raised while serving a probe route is still recorded. The exemption covers the routine per-request record and not the report of a failure, and that is exactly the boundary an implementation is likely to get wrong by placing the skip too early in the chain.
...
- [ ] 6.3 Confirm the check's timeout is strictly greater than the endpoint's own bound. If it is not, every verdict is a failure regardless of the dependency and nothing in the output says so. This is the one number in this change that is wrong in a way that looks like a working system.
