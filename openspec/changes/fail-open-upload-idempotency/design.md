...
- **[Risk]** A request that proceeds without a reservation never writes an idempotency key for its content, so that specific `VideoJob` can never be found by a future `Lookup` — not just "while it's in flight," but permanently, since no key is ever created for it. The first identical-content upload to arrive after Redis recovers will not find it either, and will create a new `VideoJob` of its own; only *that* new request establishes a key, so uploads after it dedupe normally again. → **Mitigation**: accepted; bounded to at most one extra `VideoJob`/`ffmpeg` run per distinct piece of content that was uploaded during the outage (not unbounded — the first post-recovery upload of that content re-establishes deduplication for everything after it), and consistent with this change's core premise that a missed dedup costs compute, not correctness.
