...
- [ ] 2.2a In the same job, assert per platform that all five binaries are compiled for that platform, and make the job fail when one is not. Without this the gate is green for the exact defect it exists to catch — the image builds either way. Verify the assertion by temporarily dropping `GOARCH` from one of the five builds and confirming the job goes red.
