...
**Starting version `0.0.0` in the manifest, with `bump-minor-pre-major`, `bump-patch-for-minor-pre-major`, and `initial-version: "0.1.0"` set in `release-please-config.json`.** No existing tags. Verified empirically: without these flags, release-please's default behavior treats the very first release specially and proposes `1.0.0` on the first `feat:` commit (not `0.1.0`), which doesn't fit a project still in early hackathon development. With `initial-version` set and the pre-major bump flags on, `feat:` stays a minor bump and `fix:` a patch bump while under `1.0.0`, so the project can stay in `0.x` until someone deliberately decides it's ready for `1.0.0` (via `release-as` on a future commit, or just letting normal minor bumps continue).
