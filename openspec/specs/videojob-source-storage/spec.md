...
**Application cleanup is not exhaustive, and no requirement here claims otherwise.** A job enqueued but never dispatched has no actor that can apply a terminal outcome. A worker that dies mid-extraction is recoverable by the lease sweeper, but interruption after a terminal commit and before deletion can still leave the object behind, as can a best-effort deletion failure. An expiration lifecycle rule on the `uploads/` key prefix is therefore the **only exhaustive guarantee** that source residue is reclaimed, and `docs/operations.md` SHALL describe it in those terms. Queue wait and extraction duration are not bounded in code, so a finite retention period also defines the deployment's supported recovery lifetime: operators SHALL size it above queueing, all bounded recovery attempts, lease/sweep delays, and an explicit margin, and SHALL accept that a job older than that policy may lose its source before recovery.
