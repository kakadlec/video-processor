...
**That clause is about a job already `processing` whose claim was won, and SHALL NOT be read as forbidding the requeue `videojob-worker` performs for a dispatch whose claim outcome was never learned.** The two are different operations. Recovery returns a job whose claim was *won and then abandoned* to the queue, and it is the lease's absence that authorizes it. `videojob-worker`'s requeue puts back a dispatch on which that worker performed no work; nothing in that path consults a lease, advances an epoch, or writes an outbox row.

Its possible outcomes are covered between the two requirements rather than by either alone, and the one that reaches this sweeper is **its ordinary input rather than an exception to it**. That requeue asserts nothing about the stored row: the failure may have come from the authoritative load that precedes any claim, from the claim itself, or from the probe that follows a zero-row claim. The redelivery re-runs the same load and conditional claim the original delivery would have, a case none of the reasoning above concerns — a `queued` row is claimed; a missing, `pending`, `processing` or terminal row is dead-lettered, and a `processing` row met that way belongs to its live holder or, if abandoned, to this sweeper like any other. Exactly one branch necessarily becomes this sweeper's input: if the claim committed and its result was lost, the row is `processing` and holds no lease, because a lease is acquired only once a claim is reported won; the redelivery is dead-lettered exactly as this clause describes, and the row is recovered here on the ordinary two-observation path. Stated here because the clause is otherwise the first thing a reviewer will quote against that disposition, and because that branch would otherwise look like a gap between the two requirements rather than the seam where they meet.

That committed branch spends one of the bounded requeues below, and that cost is accepted rather than compensated for: a job that exhausts the bound is committed `failed` — visible to its owner, with its idempotency key cleared — which is the outcome this capability already prescribes at the bound and is strictly better than a `queued` row nothing can reach.
...
**The bound SHALL be the single constant `domain.MaxJobRequeues` (3), and it is shared.** `videojob-execution`'s transient object-storage retry walks the same `processing → queued` edge through the same repository operation and advances the same fence epoch, so a job's epoch counts every requeue it has had, whichever caller performed it. Neither caller SHALL keep a bound of its own: a job whose storage retries advanced its epoch has fewer recovery requeues left, and that is the intended reading of one budget per job, not a leak between two mechanisms.
