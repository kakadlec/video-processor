...
**The two non-HTTP processes — `cmd/worker` and `cmd/notifier` — SHALL each expose `GET /metrics` on a metrics-only listener — port `9102` for the worker, `9103` for the notifier**, served by the standard library's server with no framework, no probe, no bearer group and no limiter, and SHALL serve nothing else on it: another path is not found, and a method on that path other than `GET` or `HEAD` is refused as not allowed. That port SHALL NOT be published to the host and SHALL NOT be routed by the ingress, and the address SHALL be a constant rather than configuration, on the same reasoning that fixes the HTTP services' `8080`. The two constants SHALL differ, so both processes can run on one host outside the compose network without the second failing to bind, and a source-level test SHALL pin that they do. The listener SHALL be bound before the process starts consuming, and a failure to bind it SHALL be fatal at startup, so a missing scrape target is never a silent state that lasts the life of the process. Its errors after startup SHALL be logged and SHALL NOT stop the process, and on shutdown it SHALL be closed within a short bound of its own that does not extend the drain of the work in hand.
...
- **WHEN** `GET /metrics` is requested on its metrics port from inside the deployment network
- **THEN** it answers `200` with the exposition, any other path on that port is not found, any method on `/metrics` other than `GET` or `HEAD` is refused as not allowed, the same port is unreachable from the host, and no request to it passes through the ingress
