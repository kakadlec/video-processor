...
- **THEN** it answers `200` with the exposition, any other path on that port is not found, any other method on `/metrics` is refused as not allowed, the same port is unreachable from the host, and no request to it passes through the ingress
