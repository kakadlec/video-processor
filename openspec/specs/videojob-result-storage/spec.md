...
`ProcessVideoJob` SHALL, after `FrameExtractor.ExtractFrames` returns successfully, store the extracted zip through `ResultStorage` under the key derived from the job's ID, and SHALL remove the local zip file afterwards whether storing succeeded or failed. A result that could not be stored is not a result, so on a storage failure it SHALL NOT report success. It SHALL return the job to `queued` for another attempt under `videojob-execution`'s transient object-storage retry, and SHALL call `FailJob` and report the failure to its caller only once that retry's bound is spent.
...
- **GIVEN** frame extraction succeeded but storing the zip fails, and the job's retry bound is not spent
