# upload-file-validation Specification

## Purpose

Define `POST /upload`'s file-extension validation contract: an unsupported extension must be rejected without the handler consuming, draining, or persisting any of the uploaded part's payload, independent of the request's declared or actual size — closing a gap where the standard `FormFile`/`ParseMultipartForm` path read the entire body before the filename was even available.
