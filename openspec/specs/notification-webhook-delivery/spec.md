# notification-webhook-delivery Specification

## Purpose

Defines what the Notification context does with a consumed terminal outcome: which preferences an event may reach and from when, the durable claim that makes one delivery out of an at-least-once queue, the payload it sends and the generation that payload carries, the signature over it, the policy that decides which destinations may be registered and dialled at all, and the bounded budget after which an outcome is recorded rather than retried forever. The process that consumes the queue and disposes of the message is `notification-event-consumer`'s; the storage behind the claim is `notification-persistence`'s; the preference being resolved is `notification-preferences`'.
...
- **WHEN** those earlier events were already handled and acknowledged while it was disabled
- **THEN** nothing replays them: no backlog is recorded while a preference is disabled and nothing rereads the queue. Enabled-ness is read when an event is handled, not when it occurred, so an event still queued when the preference is re-enabled **is** delivered — which is the same rule, not an exception to it
