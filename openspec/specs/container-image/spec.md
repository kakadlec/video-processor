...
#### Scenario: Both entrypoints are built and present
- **WHEN** the runtime image is built
- **THEN** it contains an executable for `cmd/api` and one for `cmd/worker`, each runnable on its own, and neither requires the other to be running in the same container
...
#### Scenario: Each process starts without the other

- **GIVEN** the built image
- **WHEN** either binary is started on its own with its own required configuration
- **THEN** it starts and operates, without requiring the other process to be running
