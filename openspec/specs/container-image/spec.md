...
Every binary joins the one image rather than getting one of its own for the reason the second did: they share `internal/` packages — the three HTTP services share Identity's verifier and the platform plumbing, and each pairs with a consumer over the same context's domain — and separate images would create a way for the halves of one deploy to be built from different commits of the same domain code.
...
The image now serves **five** processes, and that SHALL NOT change how any of them is configured or reached: each of the three HTTP services SHALL listen on port 8080 inside its own container, while the worker and the notifier SHALL each listen on exactly one port — `9102` for the worker, `9103` for the notifier — serving nothing but the metrics exposition `service-metrics` defines for them. That port SHALL NOT be published to the host and SHALL NOT be routed by the ingress: it exists for a scraper inside the deployment network, and a process that serves no application route SHALL NOT acquire one through it. Because the three HTTP services share that port number, they SHALL be distinguished by container rather than by port, and the compose stack SHALL publish exactly one host port **for the application** — the ingress's. A development-only support service built from an image this repository does not produce, serving no application route and present in no deployment, MAY publish a loopback-bound port of its own; the stack's three such inspection UIs are the local mail catcher `development-workflow` requires, the local metrics scraper that collects the exposition of every application process, and the local dashboard server that reads that scraper. The ports the backing dependencies publish for local development are outside this allowance and are not governed by this requirement. None of the three is an application process, and neither the scraper's own port nor the dashboard's publishes any application metric beyond what the processes it scrapes already expose inside the network. The distinction is between what the application exposes and what the local stack provides for inspection: the first is a deployment contract, the second is not deployed at all. Adding a process SHALL NOT make it a prerequisite for any other to start, in either direction — each SHALL start, run, and fail independently.
...
- **WHEN** `docker compose up --build` runs
- **THEN** the `worker` service starts from the same image as the HTTP services, running the worker binary with the environment the compose file supplies, listening only on its metrics port inside the network and publishing no port to the host

#### Scenario: The notifier service starts from the same image

- **WHEN** `docker compose up --build` runs
- **THEN** the `notifier` service starts from the same image as the others, running the notifier binary with the environment the compose file supplies, listening only on its metrics port inside the network and publishing no port to the host
