...
The stack SHALL also start a metrics scraper, so that a contributor can read every application process's exposition as series rather than as a single scrape. It SHALL come from an image this repository does not build, SHALL collect the three HTTP services on their application port and each worker replica and the notifier on their metrics-only listener, SHALL retain a bounded window rather than growing for the life of the volume, and SHALL expose its interface on a loopback-bound host port of its own under the same allowance.

The stack SHALL also start a dashboard server reading that scraper, so that what the scraper collects has a reader in this repository rather than only a query box. It SHALL come from an image this repository does not build, SHALL be provisioned from files in the repository — its data source and at least one dashboard — so that what a contributor sees is committed rather than assembled by hand, SHALL require no credential to read, and SHALL expose its interface on a loopback-bound host port of its own under the same allowance. It SHALL NOT be a source of measurement: it may only draw families the application already exposes, so adding or changing a panel is never a reason to change an application process.

The HTTP tier that command starts is **three services behind one ingress**, and the single-command promise is unchanged by that: the contributor SHALL still reach every route on one host port with no additional configuration, and SHALL NOT have to know which service answers which path. Exactly one service **serving the application's own HTTP surface** SHALL publish a host port — the ingress; the three application services SHALL publish none, which is also what allows any of them to be scaled without a port collision. A development-only support service that serves no application route MAY publish a port of its own, and SHALL bind it to loopback exactly as the ingress does; the mail service, the metrics scraper and the dashboard server are the three such inspection interfaces, and none is reached through the ingress because routing a development inspection UI through the application's own gateway would put a path on that gateway which no deployment serves. The ingress SHALL be configured so that an upload larger than its software's default request-body limit succeeds and is not buffered to the ingress's own disk.
