...
The reclaim bound SHALL be configuration, and it SHALL be required to be **at least twice** the maximum time a single claimant can hold a claim — that maximum being the attempt budget, each attempt's timeout and the backoff between them, plus the bounded retries of the resolution itself. Merely exceeding the maximum is not enough to require: the maximum is arithmetic over configured terms, and a bound a hair above it is cleared by a claimant that is slow for any reason the arithmetic does not model — a stalled write, a scheduler pause, a clock that is not the one the database reads. A bound below the budget makes concurrent outbound requests to the same destination reachable in normal operation, because the claim token fences only the database write and cannot recall a request already sent. The factor SHALL be small — headroom is not free in the other direction, since at prefetch one the same bound is how long an abandoned claim stalls the queue. The relationship SHALL be checked at startup and SHALL fail startup when it does not hold, naming both values, rather than being left as a documented convention two independently-tuned variables can silently break.
...
- **GIVEN** a configured reclaim bound below twice the maximum time one claimant can hold a claim
- **WHEN** a process using the context starts
