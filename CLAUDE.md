...
4. `ProcessVideoJob` (`internal/video/application`) is the core pipeline: `EnqueueVideoJob` → `StartProcessing` → `FrameExtractor.ExtractFrames` → `CompleteJob`/`FailJob`, driving the `VideoJob` through `pending → queued → processing → completed`/`failed`. The actual `ffmpeg` exec/zip work lives in `internal/video/infrastructure/ffmpeg`'s `Extractor`: creates a per-job scratch dir under `temp/<jobID>`, runs `ffmpeg -i <video> -vf fps=1 -y temp/<jobID>/frame_%04d.png`, globs the resulting PNGs, zips them into `outputs/frames_<jobID>.zip`, then removes the temp dir (`defer os.RemoveAll`). On success the original upload in `uploads/` is deleted too, so `outputs/*.zip` is the only durable artifact.
...
Three directories are created at startup (`createDirs`) and used by both flows above: `uploads/` (transient input), `temp/` (per-job working dir, always cleaned up), `outputs/` (durable zip results, listed by `/api/status` and served by `/download`) — file listing there is done by `filepath.Glob` against `outputs/*.zip` at request time. `VideoJob`s (created via either `/upload` or `/api/video-jobs`) are persisted in PostgreSQL (`VIDEO_POSTGRES_DSN`, required at startup like `IDENTITY_POSTGRES_DSN`).
