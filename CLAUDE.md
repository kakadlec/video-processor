...
6. `POST /api/video-jobs`, `GET /api/video-jobs/:id`, and `GET /api/video-jobs` (all bearer-authenticated, owner-scoped) wrap `internal/video/application`'s `CreateVideoJob`/`GetJobStatus`/`ListUserJobs` use cases — a preview job-lifecycle API, separate from the legacy flow above. A job created here has no processing trigger and stays `pending` forever; do not present this as a working end-to-end path (see `openspec/specs/videojob-http-api/spec.md`).

Three directories are created at startup (`createDirs`) and used as the legacy flow's only state: `uploads/` (transient input), `temp/` (per-request working dir, always cleaned up), `outputs/` (durable zip results, listed by `/api/status` and served by `/download`) — file listing there is done by `filepath.Glob` against `outputs/*.zip` at request time. `VideoJob`s created via `/api/video-jobs` are persisted separately in PostgreSQL (`VIDEO_POSTGRES_DSN`, required at startup like `IDENTITY_POSTGRES_DSN`).
