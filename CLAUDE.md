...
3. `POST /upload` (`(*videoModule).handleVideoUpload` in `cmd/api/video.go`) validates the file extension, saves the upload to `uploads/<uploadID>_<original-filename>` while hashing it (SHA-256), reserves a Redis-backed idempotency key scoped to `UserID` + content hash (see the idempotency bullet below), creates a `VideoJob` via `CreateVideoJob`, then runs it through `ProcessVideoJob`.
...
- `POST /upload` deduplicates by content: identical bytes uploaded twice by the same authenticated user reuse the first request's `VideoJob` instead of running `ffmpeg` again (`internal/video/domain.IdempotencyStore`, backed by Redis via `internal/video/infrastructure/idempotency.RedisStore`). This requires `REDIS_ADDR` at `cmd/api` startup, alongside the existing `IDENTITY_*`/`VIDEO_POSTGRES_DSN` variables. See `docs/architecture.md`'s Request pipeline section and `openspec/specs/upload-idempotency/spec.md` for the full protocol.
