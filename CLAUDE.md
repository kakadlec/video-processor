...
- **Don't confuse this with GitHub's native Dependabot alerts**, which report a much bigger number (26 as of this writing, 8 "critical") — that's a different, less precise lens: it flags every known CVE anywhere in the full dependency graph (`go.sum`), regardless of whether this app's code ever calls the vulnerable path. Most of the "critical" ones are in `golang.org/x/crypto/ssh` (agent-forwarding, host-key callback bypasses) — this app is an HTTP server that never touches SSH; that whole dependency is unused transitive weight, not a real attack surface. `govulncheck`'s 2 findings are the ones that matter (reachability-checked); use the Dependabot alerts list as a rough map of what a full dependency-version bump would also clean up, not as "26 things to fix."
