...
# as a non-root user.
#
# One image carries every entrypoint — the API, the Identity API, the worker
# and the notifier — selected by the command a service runs. They share every
...
RUN CGO_ENABLED=0 go build -o /out/app ./cmd/api \
    && CGO_ENABLED=0 go build -o /out/identity-api ./cmd/identity-api \
