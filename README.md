...
- **Notifications are opt-in.** `cmd/notifier` consumes the terminal-event queue and delivers each subscribed outcome by e-mail or by a signed webhook request. E-mail is subscribed to from the page itself — the "Notificações por e-mail" section, shown while signed in, subscribes to a failed and/or completed job; locally every message lands in the mail catcher at http://127.0.0.1:8025. Webhooks are registered through `PUT /api/notification-preferences` only; the page offers no webhook form. A subscription covers outcomes that occur after the preference was first created. A user who subscribes to nothing still has the page (which polls the job's status URL) and `GET /api/status`; absence of a preference means *not subscribed*, and there is no implicit default.
