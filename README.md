...
- **No authentication** — any client can upload and download files.
...
|---|---|---|
| `GET` | `/` | Web upload UI (inline HTML/CSS/JS) |
| `POST` | `/upload` | Upload a video file (multipart `video` field); returns JSON with `zip_path` on success |
| `GET` | `/download/:filename` | Download a processed ZIP by filename |
| `GET` | `/api/status` | List all processed ZIPs with metadata |
