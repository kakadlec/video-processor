...
Skip OpenSpec and the 3-PR split only for trivial changes (typo/comment edits) — those go straight to a single PR. A dependency bump is not automatically trivial for this purpose since it still requires a passing test run (see Definition of done); when in doubt, don't skip it.
