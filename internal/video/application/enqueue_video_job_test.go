...
func newPendingRepoJob(t *testing.T, repo *fakeVideoJobRepository, jobID, userID string) *domain.VideoJob {
	...
	job, err := domain.RestoreVideoJob(newTestVideoJobID(t, jobID), newTestVideoUserID(t, userID), filename, testSourceKey(t), "", domain.StorageKey{}, 0, "", domain.JobStatusPending, time.Now(), 0)
	if err != nil {
...
func TestEnqueueVideoJob_NoSourceKey_RejectedAndNotPersisted(t *testing.T) {
	repo := newFakeVideoJobRepository()
	filename, _ := domain.NewOriginalFilename("movie.mp4")
	job, err := domain.RestoreVideoJob(newTestVideoJobID(t, "job-1"), newTestVideoUserID(t, "user-1"), filename, domain.StorageKey{}, "", domain.StorageKey{}, 0, "", domain.JobStatusPending, time.Now(), 0)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if err := repo.Create(context.Background(), job); err != nil {
