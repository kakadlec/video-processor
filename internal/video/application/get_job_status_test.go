...
func TestGetJobStatus_OwnerRetrievesOwnJob(t *testing.T) {
	...
	job, err := domain.RestoreVideoJob(jobID, ownerID, filename, domain.StorageKey{}, "", domain.StorageKey{}, 0, "", domain.JobStatusPending, time.Now(), 0)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestGetJobStatus_CompletedJob_IncludesStorageKey(t *testing.T) {
	...
	job, err := domain.RestoreVideoJob(jobID, ownerID, filename, domain.StorageKey{}, "", storageKey, 42, "", domain.JobStatusCompleted, time.Now(), 0)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if err := repo.Create(context.Background(), job); err != nil {
